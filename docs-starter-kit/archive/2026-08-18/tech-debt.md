# 技术债清单（权威：2026-08-18 第七轮，实时活动面板/OCR 修复审查后滚动）
...
> 当日追加（v0.3.0 构建审查产出，第五轮）：TD-034/035/036 审查发现即修复（提交 8d0ff95）；
> TD-037~041 登记 open（见下）。
> 第六轮（同日）：TD-037/038/039/041 已修复（提交 1c95c32）；TD-040 评估为有意不修（deliberate，见下）。
...
| ID | 摘要 | 备注 |
|----|------|------|
| TD-033 | 窗口跨显示器移动后 DXGI duplication 不更新（仍复制旧显示器桌面），窗口裁剪为空画面，最长 30s 空窗等周期重建（ADR-007 审查发现，2026-08-18） | carried（第七轮核对，未偿）；修复方向：DxgiState 保存输出 DesktopCoordinates，窗口中心越界时立即触发重建（P2 排期） |
...
| TD-042 | 停止过渡期 stopping 状态无超时兜底——live:status stopped 事件异常丢失时右侧面板常驻"已停止"（实时活动面板审查，2026-08-18） | 修复方向：stopping 10s 超时自动清除或事件补偿查询（P3） |
| TD-043 | 实时面板时间戳用前端 Date.now() 估算，与后端会话纪元偏差（ASR 模型加载 1-3s）（实时活动面板审查，2026-08-18） | 修复方向：事件载荷携带后端时间戳（live:asr-final/live:ocr 扩展 payload）（P3，简要展示可接受） |

## 今日已偿（二轮 8 笔 + 三轮 11 笔 + 四轮 20 笔 + 五轮 3 笔 + 六轮 4 笔）
...
| TD-037 | 导入关键帧 OCR 全帧直识别：慢（单帧 1-3s）且字幕带文字混入画面要点 | import_frame.rs 区域裁剪整合：中部（5%~75%）→full + 底部（75%~100%）→subtitle，两路缩 960px + 帧间去重（六轮，提交 1c95c32） |
| TD-038 | 外挂字幕文件无大小上限，fs::read 全量读入 | parse_subtitle_file 前置 50MB 上限校验（check_subtitle_file_size 纯函数，六轮，提交 1c95c32） |
| TD-039 | 字幕投票器同组判定固定编辑距离 ≤2，长文本错字累积误判新组 | 比例阈值 max(2, 长度×15%) + 3 处错字回归测试（六轮，提交 1c95c32） |
