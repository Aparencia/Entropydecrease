...
> **交付进度（2026-08-23）**：M1（正文源多态）/ M4（DeepSeek 默认化）/ M5-精修侧（vision-exp 图片理解）/ M6（采集浮窗化）/ M7（ADR 落档）已交付；M5-帧侧（视频全帧 OCR 下线 + 关键帧存图触发解耦）/ M2（WGC 窗口级捕获）/ M3（系统级覆盖层截图）涉及实时捕获与窗口子系统，需真机验证后合入。
