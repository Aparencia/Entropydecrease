...
- **§1 单文件 ≤300 行**："过大类/文件"坏味道在本项目有明确阈值——>600 行必须硬拆；300-600 行登记豁免清单**待专项重构**（该清单由批 0 的 `0-C1` 重建为快照表；注意与拆分任务区分：`0-C2` 前端 5 个 / `0-C3` Rust 10 个拆的是 **>600** 那 15 个文件，不是 300-600 清单）。行数口径见 AGENTS.md §3.1（全部行数，以 `node scripts/line-limits.mjs` 为准）
