...
| 报错信息在代码库零匹配仍继续深挖代码逻辑 | 先核对**运行的构建与源码是否同版本**（版本号、构建产物、UI 形态）；源码中不存在的错误串只可能来自旧构建，详见 [ASR whisper.cpp 版本错位案例](../knowledge/bugs/2026-08-asr-whisper-cpp-stale-build-mismatch.md) |
