...
| app/src/components/SessionDetailPanel.tsx | 410 | 拆分进行中（批 0-C2 Task 3 逐步提交）：屏卡流 + 数据 hook 已拆出，余头部 / 精修区待拆，本行行数每步刷新；拆分前 656 行 >600 硬限（会话详情面板：原料/预览/离线精修/校对/web 早返回/术语图集；v0.20 系列 +52 后越线，预存债务随 TD-2026-09-06-G 登记，2026-09-06 实测纠偏登记值 371 过期） | 拆至：session-detail/SessionScreenCards.tsx（屏卡流 + toast/框选）· hooks/useSessionDetailData.ts（质量/术语/baseUrl + ocrBlocksByScreen memo + 4 条精修 listen + 懒触发 + 深链快照）· session-detail/SessionDetailHeader.tsx（改名/头部/降级横幅）· session-detail/SessionRefineSection.tsx（精修工具条——原登记名 SessionPass2Section 名不副实，按裁决 D2 更名）；web 早返回与质量卡/时间轴/术语表按需后置 |
