...
- **口径**：懒加载 chunk（仅动态可达的 `.js`）单独判定 —— ① **族前缀清单等式**（每个懒侧 chunk 必须归属某个已登记族，**未归族即红**）② **逐族 gzip 上限** ③ **懒侧总 gzip 上限**（另加 chunk 数上限）。三条都只许降。
- **判据键是族前缀**（`vendor-katex-*` 等）：Vite 的 chunk 名带内容 hash（不稳定）⇒ 键只能是去掉 8 位 hash 的名字加 `-`。
- **基线来源**：`scripts/lazyBudget.json` —— 首次写入值 = 2026-09-13 实测 **36 chunk / 698.86 kB**（**冻结现状，不是目标值**；写乐观「预算值」会立刻红）。
- **硬约束**：上表「页面包大小 (JS) < 200KB (gzip)」与懒侧预算是**两个独立读数** —— 懒侧判定不得影响首屏读数，两者也不得并成一个预算。
- **工具**：`node scripts/check-bundle-budget.mjs --no-build`（人类可读，**分别**打印两个 pass）；加 `--json` 得平级的 `firstScreen` / `lazyBudget` 两栏。退出码 `1` = **任一**预算超标（首屏 / 懒侧）。
