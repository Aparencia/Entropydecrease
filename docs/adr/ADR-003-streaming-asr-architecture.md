...
> 修订注记（2026-09-03，依据 ADR-030）：§决策 4 的 Silero VAD **未接线**——实际实现为能量自适应 VAD（`vad_adaptive.rs` + sherpa 端点规则，含 hangover/隔块喂入）；§决策 5 的"两遍解码"实现口径 = 实时会话端点句 SenseVoice 重打分（3s 有界、一致性校验）＋ 文件导入全窗两遍。原文保留作历史，实施口径以 ADR-030 为准。
