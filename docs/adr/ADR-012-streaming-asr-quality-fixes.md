...
已接受（2026-08-19；取证调研与方案规划完成，实施待执行）
...
- [ ] 单测（AAA）：pick_rescored 扩展接受（截断/短句/rule3 防重复/幻觉上限）、尾静音块数状态机、hangover 喂入序列、clean/dedupe 移植（原项目用例）
- [ ] 集成测试（真实模型 + 合成句尾弱音块）：句尾字不再丢失；合成"结+静音+绍"场景重打分修复
- [ ] 回归：静音期 CPU（隔块喂入在长静音段仍生效）、partial 节流行为不变
- [ ] 真机验收：3 节课对比，句尾丢字率下降 ≥80%，短句错误率下降，无新增重复段
- [ ] 取证工具 `asr_forensic` 保留（开发期诊断，不入产品包）
