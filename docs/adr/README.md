...
| ADR-021 | 笔记正文源多态——`detect_body_source` 抽象层（图文会话 OCR 进笔记正文） | 提议（v0.12.0 M1，待写） | 2026-08-23 |
| ADR-022 | WGC 窗口级捕获——三级降级链（WGC→DXGI→GDI） | 提议（v0.12.0 M2，待写） | 2026-08-23 |
| ADR-023 | 视频会话 OCR 下线 + AI 精修图片理解（含隐私授权契约） | 提议（v0.12.0 M5，待写） | 2026-08-23 |
