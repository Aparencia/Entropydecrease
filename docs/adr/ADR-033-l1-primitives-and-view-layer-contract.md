...
> **🔻 批 6 收口就地加注 · 上表行数的「三副本同步」（R33.3 / R63.2；2026-09-13，**上表与上面两段加注的原文一字未改**）**：上表是**同一张 42 文件表的三份副本之一**（另两份 = 规格 §5 的表 + [行数豁免登记](../standards/line-limit-exemptions.md) 的人工记录块），三份在批 6 收口时**均已漂移**。⇒ 本 ADR **不再抄第三份数值清单**（那等于再造一份会漂的副本）：**权威读数 = 规格 §5 的批 6 加注**（逐值 HEAD 实测 + 19 项漂移逐条）与**豁免表人工块**（T35 二次刷新）。本表只需记住 6 个关键差：`motion.css` **74 → 335** · `Text.css` **46 → 66** · `index.ts` **45 → 48** · `style-seams.test.ts` **287 → 299** · `style-contract.test.ts` **233 → 296** · `motion-coverage.test.ts` **147 → 186**；**42 个文件全部 ≤300** 对**源文件**成立（最大 **299**），唯一超 300 的是 **`motion.css` 335**（`.css` 不在行数门禁的 `SOURCE_EXT` 视野内）。⚠️ **口径缺口**：三处副本的 `<!-- line-count-src … -->` 锚点**全仓 0 个读者**（「门禁按 `files=` 重测」今日无实现）⇒ 同步只能靠人工。
...
>   - **🔻 批 4 收口后更正（收口评审 I-7，2026-09-12；**上句原文一字未改**）**：上句列出的「**11 个锚定菜单**」里有 **3 个不是 `NON_MIGRATED_14` 的成员** —— `BrowserChrome` · `GroupSidebarRow` · `NoteHeaderActions`（三者**同时不在** `ADR033_28` 与 `CROSS_LINE_34`；`BrowserChrome` 在 `.e` 里只作 `TABLE4_ALIAS`，且有一条机器断言 `CROSS_LINE_34.includes(TABLE4_ALIAS) === false` 逐字写着「别名不是成员」）；同时**缺席 3 个真成员** —— `NoteEditView` · `NoteLinkToSystem` · `RichEditorView`（三者均**非 `*Overlay`**，属那 11 的锚定/透明层）。**机器真源**（`dialogMigration.e.test.ts:115-122` 的 `NON_MIGRATED_14` 去掉 3 个 `*Overlay` 后逐字）：`GroupRowContextMenu` · `NoteEditView` · `NoteLinkToSystem` · `NoteListBatchMenu` · `NoteMoveToGroupMenu` · `NoteRowContextMenu` · `RichEditorView` · `RouteInfoPopover` · `SessionRowContextMenu` · `chat/ChatLaunchMenu` · `note-selection/SelectionActionMenu`。**复现**：`node .superpowers/sdd/2026-09-12-frontend-redesign-batch4-primitives/tmp/docfix/d4-membership.mjs`。规格 §11-2 例外表已同步同一更正（含逐行成员资格表）。**「3 个采集/预览覆盖层」与「14 条总数 = 34 − 20」不受影响**。
