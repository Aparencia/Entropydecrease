...
- `docs/tech-debt/`（审查文档，未入库）的处置：**待用户裁决**。
  > 🔻 **前向指针（2026-09-13 · 批 8 收口；上面那条原文一字未改，按当时时态读）**：该裁决**已下且已执行** —— **2026-09-13 用户裁决 U3 = a ⇒ 归档到 `docs/archive/`**，落入 [`docs/archive/2026-09-13/`](../archive/2026-09-13/)（`README.md` / `tech-debt.md` / `review-2026-09-11.md`）；原登记项就此结清，**本 ADR 的既有结论一字未改**。
...
- 「会话详情头改粘性」与「笔记工具栏三层合并为单行」（规格 §6.2「本次改动」列的两条）：**批 3 未做**，登记给**批 4/批 5**。
  ...
  > ⚠️ **加注⑦ · T18 的处置（2026-09-12 批 4 收口 · S-4 结清；上面两条加注的原文一字未改）**：🔴 控制方裁决给了两条走法 ⇒ **T18 选后者：按批次归属登记为「批 5（视图层样板）」的 follow-up，并明确它**不是**批 4 的交付。**
  > - **事实核对（两条都可复现，不是推断）**：批 4 的 **48 个提交**里**没有**任何一条做这两件事 —— ① `git diff 4905d4d8^..HEAD -- app/src | Select-String '^\+.*(sticky|粘性)'` ⇒ **空**（`components/session-detail/SessionDetailHeader.tsx` **确实在本批改动集里**，但那是 T15/T16-B 的 `StatusLine`/`Text` 迁移，**与粘性头无关**）；② `git show HEAD:app/src/pages/NotesPage.tsx | Select-String 'Toolbar'` ⇒ **空**（三层工具栏**一处未合并**）。⇒ 上句「登记给批 4/批 5」里的**批 4 一支已空转**（T0 的 S-4 警告「登记 ≠ 承接」**第二次应验**）。
  > - **归属：批 5**（视图层样板 = `viewRegistry` / 会话 4 视图 / 笔记 3 视图 / 惰性挂载）—— 两条都是**页面/面板结构**改动，与批 5 的对象-视图矩阵同面；**若批 5 未做 ⇒ 回落批 8 治理收口**（与批 3 follow-up #15 的处置一致）。
  > - **代价（若一直不做）**：详情头不粘性 ⇒ 长转写滚动后失去上下文锚点；工具栏三行不合并 ⇒ 垂直空间被占（属**观感/可用性**债务，不是正确性债务）。
