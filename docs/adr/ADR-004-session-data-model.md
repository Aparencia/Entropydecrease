...
- 会话产物可一键转为笔记（复用 v0.1.0 的 `save_draft_as_note` / 拼接逻辑——**该命令已于批 1 删除，2026-09-12；现行通道是 `session_to_note`（`commands_session_note.rs`），复用关系不变**），但会话本身独立存在（可多次转笔记、可删除会话不影响已有笔记）
