# 技术债清单（权威：2026-08-19，九轮滚动——会话 11/12/22 实测分析 + ASR 取优整合后）

> 本清单为当前唯一权威债务清单，归档日滚动更新；旧归档清单仅历史追溯。
> 来源：八轮清单滚动——TD-040 维持 carried（deliberate 有意不修）。
> 九轮滚动（会话 11/12/22 转写时间轴差异实测分析 + ASR 取优整合，提交 693dc6d）：
> 已偿 1 笔（TD-2026-08-19-C），见下。

## 未偿债务

| ID | 摘要 | 备注 |
|----|------|------|
| TD-040 | tauri.conf.json bundle.resources 未含 ffmpeg——生产安装包无捆绑 ffmpeg（v0.3.0 审查，2026-08-18） | carried（deliberate 有意不修）：resources glob 对缺失目录构建失败；捆绑 ffmpeg（~80MB）与安装包体积权衡留待体积策略；开发期由 download-ffmpeg.ps1 + PATH 覆盖（ADR-008 风险项，保持观察）。九次核对（2026-08-19）：F4-1/F4-2/捕获修复/merge-then-split/取优整合均未涉模型分发/捆绑，维持 carried |
...
| （TD-2026-08-19-C） | 会话 22 转写内容缺失（音频 54.6s 全程有语音，DB 仅转写 9.7-21.9s，~80% 内容未转写）+ 会话 11/12/22 转写时间轴差异大（用户实测，medium-high） | 实测分析（离线转写 vs DB + 文件回放复现 + 推理耗时实测）定位为版本时序 + 运行时环境问题；防御性取优整合（提交 693dc6d）：重打分有界等待（3s 超时）/停止 drain（8s 宽限）/VAD 阈值上限（0.05）/扩展接受放开至 rule3 段/健康判定固定阈值；721 单测全绿；遗留观察：会话 22 确切运行时机制（exe 版本）未完全还原，待真机重测对照（九轮登记） |
