# 技术债清单（权威：2026-08-19，七轮滚动——用户实测回归修复后）

> 本清单为当前唯一权威债务清单，归档日滚动更新；旧归档清单仅历史追溯。
> 来源：六轮清单滚动——TD-040 维持 carried（deliberate 有意不修）。
> 七轮滚动（用户实测回归修复：实时转写整段合一——链式合并无上限）：
> 修复 1 笔（提交 890a066，见下）。

## 未偿债务

| ID | 摘要 | 备注 |
|----|------|------|
| TD-040 | tauri.conf.json bundle.resources 未含 ffmpeg——生产安装包无捆绑 ffmpeg（v0.3.0 审查，2026-08-18） | carried（deliberate 有意不修）：resources glob 对缺失目录构建失败；捆绑 ffmpeg（~80MB）与安装包体积权衡留待体积策略；开发期由 download-ffmpeg.ps1 + PATH 覆盖（ADR-008 风险项，保持观察）。七次核对（2026-08-19）：F4-1/F4-2/捕获修复/链式合并上限均未涉模型分发/捆绑，维持 carried |
...
| （TD-2026-08-19-B） | 链式合并无上限——连续语音（句间停顿 <600ms）下 sherpa 端点只由 rule3（8s）触发，全部硬切段无限挂起合并：实时流不推送（前端只显示 partial 一行）、停止时 flush 整段兜底落库（用户实测"实时只显示一行 / 全部语音集中一段"，medium-high） | `MAX_MERGE_CHAIN=2` 上限（最多 3 段合一，覆盖 13.wav"同句三刀"）+ 超限强制落库推送 + ADR-012 决策记录；修复提交 890a066（七轮登记） |
