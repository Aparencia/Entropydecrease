...
> + 存储域遗留 7，见下）。
> 十五轮补记（存储域深度审查报告交叉核对）：即修 1 笔（image_crop 白边扫描注释
> 更正，提交 8a994eb）；补登记 6 处（M4 并入 TD-D；M6/M7/L6 并入既有观察项；
> 新增观察项 ST-L8/ST-L9）。交叉核对：H1/H2/M1/M8 已在 952386b 修复（cer_bench
> pub mod、备份改名报错+回滚、三类事件写入方、add_event 事务化），M2/M3/M5/M9/
> L2/L3/L5 已在前轮登记。
...
| TD-2026-08-19-D | image_stream_store 已交付未接线（REQ-110 图像流存储层/REQ-123 步骤图卡配图/REQ-088 图注影子层）：ImageStreamStore 零生产调用——跟练/白板/编程等 ImageFirst 档案只放宽图集预算，时间轴帧序列未落库，步骤卡"有卡无图"，图注无处落（v0.7.0 审查发现，medium） | open：REQ-110/123/088 验收依赖；接线点明确（live_frame_process 帧归档处创建 ImageStreamStore + record，analysis 产物模板消费 step_frames），待 M3 平台图像后续迭代接线；接线前不新增债务（机制先行已登记豁免）。接线前先修内部缺陷（深度审查 M4）：Low 帧去重单值 ahash（A→B→A 往返漏判，需 FIFO 多指纹）、bgraw=None 帧占位刷屏（ahash=0 守卫失效）、空帧丢边界/同帧重复标记覆盖、persist 每帧全量序列化 O(n²) 写放大、重开不恢复 last_ahash |
...
| （审查 L11） | 前端 types.ts SessionDetail 缺 events/region_kind 字段（serde 契约漂移，M1.5 起） | 前端当前不消费 events 字段（详情页未显示信号事件），类型补齐随前端迭代。EventKind IPC 序列化 PascalCase 与 DB kebab-case 双表示（深度审查 M7）——补类型时统一 `#[serde(rename_all="kebab-case")]`（对齐 DB 口径），保留 `#[serde(default)]` 兼容旧缓存 |
| （审查 S-L10） | foreground_switch 事件载荷只存裸 hwnd（无窗口标题）——消费端辨识度低 + 句柄可枚举隐私面（low） | 事件为本地落库（数据不出本机）；hwnd 载荷为隐私正面有意偏离（标题敏感，深度审查 L6 通过项）——设计文档 §四 载荷表待同步；窗口标题增强随 REQ-128 时间线 UI 迭代（标题截断 + 敏感词策略先行） |
...
| （审查 ST-L3） | REQ-131"可回退"仅查询无动作——model_disk_overview 只读展示（low） | 前端面板（TD-2026-08-19-E 接线时）配套"清理"动作；验收条款含前端迭代。list_versions 仅测试引用（模块级 dead_code 豁免掩盖，深度审查 M6）——删除或接线 |
...
| （审查 ST-L8） | image_caption/ocr_confusion 影子层验收未落地——ImageCaptionShadow 无表、ConfusionProfile 无采集端 record 调用（REQ-088/120 依赖）（low） | 逐项定性（接线/顺延）随 REQ-099 治理迭代；接线后去掉模块级 dead_code 豁免改逐项 allow（深度审查 L4：已接线模块豁免应收紧；list_events_by_kind"供 M2 按类型消费"理由已过时——M2 实际内存过滤，注释待更正） |
| （审查 ST-L9） | backup_restore 深度方案未落地——恢复中闸门/重启式换入（Unix 改名成功场景活连接写旧 inode 数据分裂余项）+ -journal 残留（low） | 当前靠"改名失败明确报错引导关闭应用 + 解压失败回滚改名"兜底（已偿 H2-b）；重启式恢复设计随备份安全加固迭代 |
