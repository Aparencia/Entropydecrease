...
## 本批补充（八轮：pain-points-v2 feed 地形下卷沉淀 + 实验内容移除，2026-08-21）

**归档判定**：本批无文档归档——`docs/product/pain-points-v2.md` 为活跃产品规划文档（持续维护）。

**工作摘要**：
- **feed 地形下卷沉淀**（提交 2d494ab）：pain-points-v2.md 改为双卷结构——上卷（容器地形）+ 下卷（feed 地形·短视频学习选题三轮头脑风暴收敛轮纪要）：同步器模型（离合器：容器叙事超前／feed 能力超前的失同步双向性）+ 晚到三发现 N23–N25（觉察滑差／学期结算蒸发／常模缺失，续接主编号）+ feed 八状态生命周期表 + 一基座（测量层）三柱子（显影／时机／容器）+ 四层指标（北极星：有证据的校准）+ v2.1 主理论四条补丁；附录 A 增补 feed 术语条目
- **实验相关内容移除**（提交 5e37b79）：整章删除上卷原第十部分「实验序列」（7 裁决实验 + 熔断器 + 10 份访谈）与下卷原第二十四部分「实验序列增补」（三臂实验 + 熔断链）；两卷路线图 Phase 0（验证期）移除；正文零散实验提及改写（B 堆待验证／第四风险处置置空等）；全卷章节重编号连续化（上卷一~十四，下卷十五~二十六）+ 交叉引用同步（v2.0 §12.1→§11.1、见第二十一→二十部分）；全文档关键词（实验／访谈／熔断／基线调查／Phase 0）验证 0 残留。**注**：七轮工作摘要中「实验序列（7裁决实验 + 熔断器）…Phase 0-4」为该时点文档状态的历史快照，本批起该内容已从活跃文档移除
...
## 本批补充（九轮：pain-points v3 融合裁决卷沉淀，2026-08-21）

**归档判定**：本批无文档归档——`docs/product/pain-points-v3.md` 为活跃产品规划文档；v2 与 v3 并行生效（v2 定义两地形理论，v3 裁决共存方式），v2 不归档。

**工作摘要**：
- **v3 融合裁决卷沉淀**（提交 0b26eec）：新建 `docs/product/pain-points-v3.md`，对 v2 上下卷分叉关系正式裁决。三个裁决：①地形定义——边界在**结构承载性**（结构在内容里→容器；结构在行为里→feed），平台/时长/学习声明分别降格为适配器/启发式/路由开关；②产品形态——一个产品、一个产物模型、两条地形管线（共享引擎 + 独立管线，否决进程隔离），不拆版本线；③融合点——产物层笔记组：结构密度路由（高密度自成一组/低密度归 DomainTag 主题组）+ 组三条设计契约（领域粒度拒大类/组是学习单元非档案盒——复习测试绑定到组/组结算防沼泽）。配套：下卷资产三档重定位（一期吸收交接+结算／二期增强显影盲测常模／同步器理论保留）、指标融合（北极星：组学习循环完成；身份指标：第二组启动率）、Phase 1-4 路线图、v3 特有 Pre-mortem 五项；product README 索引登记 v3 并更新 v2 描述
