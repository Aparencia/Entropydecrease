...
- **「应用/决策」是整合型零竞品带**：把「概念/模型 ↔ 决策日志 ↔ 审计」做成产品级记账结构的产品**不存在**（孤立决策日志 App 已存在，见[知识体系类软件调研](./competitive-analysis-knowledge-system-software.md)——证明需求真实，整合是差异化）。
...
| Q2 | 有没有人做"应用/决策"环节？ | 独立决策日志 App 存在（[Decisio/Verdict](./competitive-analysis-knowledge-system-software.md)，孤立无整合）；**产品级"决策日志+审计+概念引用"整合为空**——最接近的是反思日记类（Day One、flomo 回顾：无概念↔决策结构）与白板类（Heptabase：空间组织非记账） |
