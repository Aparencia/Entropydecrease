...
const normalizeEol = (text) => text.replace(/\r\n/g, '\n'); // core.autocrlf=true：不归一化会在 Windows 检出上恒定误报（同 批 0-A tokens.drift.test.ts）
if (!existsSync(TEMPLATE_PATH)) {
  console.log('⏭️ 模板自检跳过：docs/scripts/docs-check.mjs 不存在（非 Starter Kit 仓库属正常情况）');
} else if (normalizeEol(readFileSync(SELF_PATH, 'utf8')) !== normalizeEol(readFileSync(TEMPLATE_PATH, 'utf8'))) {
  exitCode = 1;
  console.error(
    '\n❌ 模板源与实例已分叉：docs/scripts/docs-check.mjs ↔ scripts/docs-check.mjs —— ' +
      '请把改动同步到另一份（模板是 Starter Kit 的源，实例是本项目实际运行的那份）',
  );
} else {
  console.log('✅ 模板源与实例一致');
}
