...
> **状态**: 实施中（2026-08，M0 已偿 TD-033；M1 已实施——EP 改 CUDA（ADR-009）；M2 已实施——动态 ROI；M3 已实施——灰转黑静默修正 + hash 短路；M4 已实施——预算制调度（VAD 旋钮参数化 + 降级档）+ CPU 负载监测 + OCR LRU 缓存；REQ-033 真机验收待用户执行）
