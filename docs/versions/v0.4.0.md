...
> **状态**: 实施中（2026-08，M0 已偿 TD-033；M1 已实施——EP 改 CUDA（ADR-009）；M2 已实施——动态 ROI；M3 已实施——灰转黑静默修正 + hash 短路；M4 已实施——预算制 + 负载降级 + OCR 缓存；M5 已实施——词表闭环 + 课件预热；M6 已实施——预处理链（AGC/削波/动态阈值，默认关）+ 声道选优 + 标题信号确认入库；REQ-033 真机验收待用户执行）
