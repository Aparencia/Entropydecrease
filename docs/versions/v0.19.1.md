...
- Rust：kb_prompt 10 用例 + db_ai_chat（retrieval/meta_json 迁移幂等 roundtrip）+ ai_settings（kb_qa_gate 三态矩阵）——`kb_`/`ai_settings`/`db_ai_chat` 过滤全绿
- 前端：kbHits 解析/词提取/标签 7 用例 · CitationChips 渲染/点击跳转带词/碎片禁点/空命中 4 用例 · tsc 零错误
- 全量 cargo test / vitest / clippy 终验待收尾执行
