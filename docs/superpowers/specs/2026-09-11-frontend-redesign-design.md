...
> ② **`--ed-shadow-1` / `--ed-shadow-2` 的具体值尚未定，本批不产出**。`--bg-raised` 亮档的分层依赖它 —— **必须在批 0-D（原语层 Modal）落地前定值**，已登记为待办。**批 4 迁移时不得临时硬编码阴影**。
...
| 字阶 | 25/600 · 17/600 · 15.5/1.9 · 13/20 · 12/18 · 11.5/16 mono（**下界 12px 约束正文与界面文字；`11.5/16 mono` 是唯一被点名的例外** —— 等宽用于时间码与元数据，11.5px 等宽的可读性高于同尺寸黑体。10px / 11px 一律消灭） |
