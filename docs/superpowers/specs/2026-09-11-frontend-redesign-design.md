...
2. 手写弹层 20 → 全走 `Modal`；z-index 不同值 17 → **≤6 档**；`role="dialog"` + 焦点陷阱 **20/20**。
   > **批 3 未涉及，只登记**（2026-09-12）：本批**没有迁移任何弹层**（非目标 2：不得 import `ui/primitives`），20 个弹层与 17 个 z-index 值**一个未动**。批 3 只新造了两个**消费六档标尺**的面板（`CommandPalette` 用 `zIndex("palette")`、AI toast 用 `zIndex("toast")`），并**顺手登记**了 `App.tsx` 对话面板残留的**裸 `900`**（批 4 迁移点，见批 3 计划 §收口回写 §批 4 的迁移点）。⇒ 本条的判据与计数**不变**，验收归批 4。
