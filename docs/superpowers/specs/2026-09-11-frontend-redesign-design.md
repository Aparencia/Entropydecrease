...
> **🔻 批 6 收口就地加注 · 红线 6 的第三处例外 E3 / E4 / E5（R0.1 第 1 条①② · R5.4 / R5.5 / R5.5-b；2026-09-13，**上面原文一字未改**）**：
> 上面 `:127` 的「除下列**两处**经批准的例外」与 `:129` 的「经批准的**两处**例外（各自需登记原因 / 影响面 / 回滚方式）」**按本加注读作三处**；上表两行**原文保留**，新增三行逐列如下（依据 = 用户 2026-09-13 裁决「批 6 范围 = B · 连承载面一起建」⇒ 三条硬后果第 1 条）。三条**共同约束**（逐条适用）：**只读** / **不新增表** / **不改既有字段与既有 SQL 文本的语义** / **零迁移**（`flashcards` 建表后零 `ALTER TABLE`；`CARD_COLUMNS` 不动）；**都不触碰 AGENTS.md §10 的「SQLite schema / 迁移」面**。
>
> | # | 例外 | 原因 | 影响面 | 回滚 |
> |---|---|---|---|---|
> | **E3** | `Flashcard` += **只读字段** `intervalDays`（camelCase；Rust 4 文件 / 5 处 + 前端可选字段） | **真实间隔前端今天拿不到，但后端已经算出来了**（`scheduler.rs:69` 的 `interval_days: f32` 与 `:99` 的 `max(1.0)`），而 `review_card` 的返回只给 `state_json` / `due_at`、写库只有 `UPDATE flashcards SET state_json = ?1, due_at = ?2` ⇒ **DB 无 interval 列**；而 `stateJson` 的契约是「后端调度契约，**前端透传不解析**」⇒ 前端**不得**用 `dueAt` 差分或字符数**自造**「真实间隔」 | `Flashcard` 是 **6 条命令的共享返回类型** ⇒ 字段会同时出现在 `list_due_cards` / `review_card` / `quiz_group_cards` / `create_model_card` / `list_group_cards` / `model_card_from_note` 的返回里（**本批只消费前两处**，其余四处如实登记为「顺带变化」）。前端 `app/src/types/notes.ts` 加**可选** `intervalDays?: number`（零夹具改动）。🔴 **两个精度域必须在类型注释里显式声明**：`review_card` 路径 = **精确值**（当场覆写 —— ⚠️ **陷阱**：该函数用 `..card` 结构体更新语法，**不覆写会漏出旧值**）；行派生路径 = **整天粒度**（`row_to_card` 由 `dueAt − stateJson.lastReviewMs` 反推 `max(round(…),1)`） | 删字段 + 还原 5 处（`types_note.rs` · `db_flashcards.rs`×2 · `commands_flashcards.rs` · `types_contract_tests.rs` 的**追加行**）+ 删前端可选字段。**零迁移、零 SQL 变更** ⇒ 不影响任何已落库数据 |
> | **E4** | 新增**只读** IPC `session_audio_path`（**命令注册 312 → 313**） | **音频真的落盘且已接线，但前端拿不到路径**：`audio_store.rs` 写 `{data_dir}/session-audio/{session_id}.wav`（16 kHz 单声道 PCM16，44 字节 RIFF 头，~115 MB/小时）；播放三件套**已现成**（`tauri.conf.json:24` 的 CSP 已含 `media-src … asset:`、`:25-28` 的 `assetProtocol.enable=true` + scope 覆盖 `app_data_dir`）⇒ 🔴 **本批零 `tauri.conf.json` 改动**（AGENTS.md §10 该审查面**不触发**） | **本批唯一的新增 IPC**：入参 `session_id: i64`；出参 `Option<SessionAudioRef>`（字段只许 `path` / `aligned` / `durationMs` —— `aligned` 自证量必须到 UI，`Option<String>` 装不下）；计数 **312 → 313**。🔴 **安全三条**：① 必须校验入参 ② 路径必须限定在应用数据目录内（**不许**把任意路径透传给 asset protocol）③ **不许前端 `fetch()` 整取音频** ⇒ **只许 `<audio src>`**。⚠️ **适用范围硬边界**：**只有实时采集会话有音频**；导入会话恒 `Ok(None)` | 删命令 + 删注册行 ⇒ 计数回 **312**；前端删调用点即回到「只读时间轨、无音频播放」。**零依赖变更、零 `tauri.conf.json` 改动** |
> | **E5** | **WAV 轴 ≡ 会话轴**：写块路径按 `chunk.timestamp_ms` 的**空档等长补静音** + **失效安全** + **`aligned` 自证量** | 今天 `segments[].start_ms / 1000` **不能当** `audio.currentTime`：`start_ms` 是会话纪元墙钟 − `total_paused_ms`，而 WAV 轴 = **已写样本数 ÷ 16000**、纯追加、无任何静音填充。⚠️ **暂停本身不产生偏移**（端点 `Stop` ↔ 时间戳冻结严格抵消）；间隙来自**静默窗根本不产包** ⇒ 偏差 **D2 最危险且无上界**（大量静默的课程录音可达**分钟级**）。「只暴露 `first_sample_ms + samples_written`」的最小方案**只修 D1**、会交付一个「看起来对、实际会漂」的播放头 | `live_session*.rs`（**AGENTS.md §10 名单内** ⇒ 该任务必须显式执行 §10 审查记录）· `audio_store.rs`（`write_chunk` 加 `timestamp_ms` 入参 + 对齐簿记 + `finalize` 写 sidecar）· `live_session_loop.rs` **一个**调用点 · 新纯函数 `audio_align.rs` + 测试 · `audio_store_tests.rs` 的既有调用点**改签名**（必要连带，**只改调用形态、不改任何期望值**）· **ADR-013 就地加注**。**失效安全语义**：时间戳缺失 / 非单调 / 超大空档（> `MAX_GAP_MS` = **10 分钟**）⇒ 退回纯追加并置 `aligned = false`（**绝不静默分歧**），**不丢样本、不阻断会话主链路**。🔴 **历史录音必须如实区分**：本修复之前的 WAV **不保证对齐** ⇒ UI 对无基准的录音**不得假装精确**；**禁止**用恒为 `true` 的字段充当该标记 | 还原 1 个签名 + 1 个调用点 + 删 2 个新文件 + 停写 sidecar（**补静音只影响新录音**，既有 WAV 一字不动）；`aligned` 随命令一同删除。**零新增依赖、零迁移、零新数据面** |
>
> ① 🔴 **`cargo test --test app_lib_tests` 本批真跑**（R0.1 第 1 条③，**不得**把「未跑」写成「已跑」）：`running 2335 tests` → **`test result: ok. 2329 passed; 0 failed; 6 ignored`**，**exit 0**（读数出处：`task-34-report.md` §3）。② 例外 E4 的落地实测：注册表 **313 / 313 / 0**（定义 313 / 注册 313 / 重复 0）。③ 逐条细则与「①域（精确值）不落库」的完整口径见 [`ADR-035` 的「后端契约例外」节](../../adr/ADR-035-l4-motion-grammar-and-engine.md)（本批定稿）。
...
> **🔻 批 6 收口就地加注 · 条件 ③ 的去向 + 环境层第 ③ 件的落点归属（R6.2 · R11.3 · R12.4 · R62⑤；2026-09-13，**上面原文与批 4 加注一字未改**）**：
> - 🔴 **`ink-4` 的盘点终态 = 保持 0 生产调用点**：上面批 4 加注的结论在批 6 **未被推翻**（本批**零动作**、未承接「审校模式」）⇒ 条件 ③ 的**去向 = 批 7/8**（`ADR-035` 的「登记」节逐字同名一条）。**本批不为此新增任何例外。**
> - 🔴 **口径更正（R11.3 的落点归属，勿混用）**：环境层第 ③ 件「**未确认段落墨度极缓慢起伏（幅度 2%）**」的**落点不是 `ink-4`**，而是 `lowConfidenceClass()` + 类名 `ed-text--low-confidence`（落点 `app/src/components/structuredBlocks.ts`，消费者见下）—— `ink-4` 与它**没有实现关系**。⚠️ **兑现度如实登记**：该件的**生产调用点终态 = 1 处**（`components/session-detail/SessionRawView.tsx:137`），而 **R12.4 要求 ≥2 ⇒ 差 1**（T25 / T27 均未补第 2 处；`views/session/SessionTriTrackView.tsx` 只剩 5 行余量）⇒ 本批**只登记、不补**（这是 R12.4 设计来抓的东西，**不许**用 T13 的 CSS seam 充数）。
> - **环境层四件的逐件兑现度**（R11.3 逐字「不得把『落 seam 但 0 调用点』算作已交付」）：① 探针摆动 = `Loading.css` 的 `.ed-probe`（**既有**，批 6 只对齐名单）· ② 采集脉冲 = seam 就绪、**承载面未落**（`LiveBar` 用波形条阵列，未做脉冲节拍）· ③ 未确认段落墨度起伏 = **1 处调用点（要求 ≥2，差 1）** · ④ 到期刻度微光 = 落在 `DueScale` 的静态形态上（**未做循环微光**）。🔴 另：「**丰富**」档只兑现了**频率**提高，**幅度上限未动**（真正依据 = 本规格 §8.6.1 第 1 条逐字「环境层**幅度上限不动**」，**不是**「CSS 无原语」——逐档 keyframes 是可行的）。

> **🔻 批 7 收口就地加注 · 「审校模式」的**规格定义**（C9.18；2026-09-13，**上面原文与批 4 / 批 6 加注一字未改**）—— 🔴 **本节只定义，不实现**：**
> - 🔴 **状态逐字：未实现。** 本节补的是**条件 ③ 的前置定义**（「进入审校模式」到底指什么）；**批 7 不写一行实现代码**。⇒ 登记为**批 8 输入**（§11-7 与批 8 的输入清单里**按名登记**）。
> - **① 入口（哪个面、怎么进）**：**阅读面（`notes` 对象）的全局开关**，载体 = `App` 壳上的一个**模式位**（`<html>` 上的 `data-proofread="on"`）+ 一个可发现的入口按钮（`⌘/Ctrl+Shift+R`，与既有 `⌘K` / `Esc` 同族的键盘纪律）。**不进**视图注册表（它不是第四种「视图」，而是**所有视图共同的面**）⇒ 与 §7.2 的视图计数**无关**。
> - **② 作用域**：**当前阅读面的全部文本节点**（含 `NotesReadingColumn` 的每一种视图、卡片流行、印样正文；**不含**编辑器（`RichEditorView`）与弹层外的外壳 —— 编辑态已有自己的墨度语义）。作用域**随焦点走**而不是随路由走：切到别的对象 ⇒ 模式**保持**（它是「我此刻在审读」的用户意图），但**只影响当前挂载的阅读面**。
> - **③ 退出**：三条**等价**出口（都必须能出）：**再按同一个快捷键** · **点同一个入口按钮** · **`Esc`**（同 `Modal` 的退出纪律）；退出后墨度**立即**回到未进入前的档位（不得留任何中间态）。
> - **④ `data-*` 形态（照 `data-motion` / `data-tone` 的范式）**：模式位写在 **`<html>` 根元素**上、**不是**类名（与 §8.3 的「载体逐字 = `data-*` 属性，禁止用类名区分」同一条纪律）——
>   · 属性名 **`data-proofread`**，取值 **`"on" | "off"`**（缺省 = `off`，且**不落属性**——与 `data-motion` 的缺省形态一致）；
>   · 墨度侧**不新增 token**：审校模式是**档位覆盖**（条件 ③ 逐字「全部升到 ≥4.5:1」），不是第五档墨度。
> - 🔻 **批 8 T11 就地加注 · 第 ④ 条的**属性名消歧（2026-09-13；**上面第 ④ 条的原文一字未改**）**：第 ④ 条原文的 **`data-proofread`** 与既有的「**LLM 文本校对**」**同名不同物** —— 后者已占用 `proofread` 一族（`src-tauri/src/commands_proofread.rs` · `components/ProofreadPanel.tsx` · `proofread_estimate` / `proofread_run`；**T11 实测，域 = `git ls-files` 1,628 文件，时点 `4041eab5`：子串 `proofread` = 250 处 / 38 文件**）⇒ 🔴 **本模式的属性名取 `data-proofread-mode`**（控制方 §2 **B2** 逐字），**不得**用 `data-proofread`。
>   ⚠️ 第 ④ 条原文出现 `data-proofread` 的**三处**（本注插入前实测 = 本文件 `:235` / `:239` / `:241`，域 = 规格全文）**一律按本注改读**为 `data-proofread-mode`；**取值口径一个字不改**（两态 `"on" | "off"`，缺省 = `off`，且**不落属性**）。
>   ✅ **已落地的载体（批 8 T9）**：`app/src/shell/proofreadMode.ts` 的 `PROOFREAD_MODE_ATTR = "data-proofread-mode"`；CSS 侧消费形态 = `html[data-proofread-mode="on"]`（`app/src/ui/proofread.css`，由 `main.tsx` 在 token CSS **之后** import）。T11 实测：`data-proofread-mode` 形态全仓 = **46 处 / 6 文件**（域 = `git ls-files`，时点 `4041eab5`）⇒ 名字已统一，**没有**任何生产代码再写 `data-proofread`。
> - **⑤ 实现层（一次覆盖，不改调用点）**：**纯 CSS 一层**完成 —— `html[data-proofread="on"]` 下把 `--ed-ink-4` 的值**指向 `--ed-ink-3`**（`ui/tokens.css` 的变量覆盖；`.ed-text--ink-4` 的规则一个字不用改）⇒ 「全部升到 ≥4.5:1」由**变量重绑**一次兑现，调用点零改动、零逐处判断。
> - **⑥ 今日实测（本节的两条读数）**：`tone="ink-4"` 的**生产调用点 = 0 处 / 0 文件**（**正控** `tone="ink-3"` = **201 处 / 94 文件** ⇒ 非「假 0」；**负控** `zzz_no_such_symbol_zzz` = 0）；`ink-4` 的**任意形态**（含类型联合 / token 定义 / 对比度判据 / 注释）在生产侧 = **5 处 / 4 文件**，**全部**是「`ink-4` 是过渡态、不得承载唯一关键信息」这类**禁止性**表述或定义，**没有一处是把 `ink-4` 用在元素上**。仪器：`.superpowers/sdd/2026-09-12-frontend-redesign-batch7-unwired/tmp/t20/probe-spec-readings.mjs`（只读；口径 = `app/src/**` 的 `.ts`/`.tsx`，PROD 排除 `*.test.*`）。
> - 🔻 **补时点（2026-09-13 · T21 补轮；上面那条读数的**数**不动，只补时点与漂移声明）**：上面写的正控 `tone="ink-3"` = **201 处 / 94 文件** 是**某个时点**的读数 —— 🔴 **它在 `fb73e313` 那棵树上逐字可复现**（侦察单元已证），此后 `b72d4709`（T20 的六条展示面）与 T23 又加了文件 ⇒ **数继续变**。🔴 **T21 于 `53bd0759` 用自己的仪器复测**（入库域 = `git ls-files`；口径 = `app/src/**` 的 `.ts`/`.tsx` 内**子串 `tone="ink-3"` 的出现次数**）：**PROD（排除 `*.test.*`）= 214 处 / 99 文件** · **TEST = 2 处 / 1 文件** · **全树 `app/src` = 216 处 / 100 文件**。
> - 🔴 **纪律逐字**：**该数随时点变化，引用必须带时点（并带仪器与域）**。⚠️ **不得**把 `216/100` 当成「真值」—— 它**同样会漂**；也不得把 `201/94` 当错值 —— 它是**时点值**。（本轮另见批 7 的「贴边件读数必须带时点」与「数字带时点」两条纪律，同源。）
> - 🔴 **不得合并结案**：条件 ③ 是**环境层第 ③ 件之外的另一件事** —— 不能用「做了 `lowConfidenceClass`（未确认段落墨度起伏）」宣称本条已兑现（原文 §4.3 批 6 加注已逐字记载：`ink-4` 与它**没有实现关系**）。批 7 对 `lowConfidenceClass` 的加宽（生产调用点 1 → 2）**不改变**本条状态：**仍是未实现**。
> - 🔻 **批 8 T3 就地加注 · 第 ⑤ 条的**覆盖落点**更正（2026-09-13；**上面第 ⑤ 条的原文一字未改**）**：第 ⑤ 条逐字点名的落点 `ui/tokens.css` 是**生成物** ⇒ **不得**在它里面写这一层覆盖。🔴 **T3 实测**：`app/src/ui/tokens.css:2` 逐字「**由 scripts/gen-tokens.mjs 生成，请勿手改。**」· `:3` 逐字「**手改会被 src/ui/tokens.drift.test.ts 判失败。**」；守卫本体 = `app/src/ui/tokens.drift.test.ts:25-28`（`it("tokens.css 与生成器输出一致（手改即失败）")` 断 `expect(normalizeEol(onDisk)).toBe(normalizeEol(renderAll().css))`）⇒ 手改**必红**。
> - ⇒ 🔴 **批 8 的实际落点 = 新建 `app/src/ui/proofread.css`**（控制方 §2 **B1** 逐字）：在**根元素模式位**下把 `--ed-ink-4` 的值**重绑到 `--ed-ink-3`**，**同时不动** `app/scripts/gen-tokens.mjs` 与 `app/src/ui/primitives/Text.css`。⚠️ 第 ⑤ 条原文的「`ui/tokens.css` 的变量覆盖」一句**按本注改读为「专用 CSS 文件（`app/src/ui/proofread.css`）的变量覆盖」**；**本注只改落点、不改机制** —— 第 ⑤ 条的机制仍是「**纯 CSS 一层、调用点零改动、零逐处判断**」。
...
> **🔻 批 6 收口就地加注 · 这张 42 文件表的**三副本同步**（R33.3 / R63.2 · T16 的二次刷新；2026-09-13，**上表与上面 `:260-262` 的原文一字未改**）**：
> 上面 `countLines()` 表与 [`ADR-033`](../../adr/ADR-033-l1-primitives-and-view-layer-contract.md) §1 的表、[行数豁免登记](../../standards/line-limit-exemptions.md) 的人工记录块是**同一张 42 文件表的三份副本**；`:211` 的刷新规则要求三副本同步，而三份在批 6 收口时**均已漂移** ⇒ 本节按 **HEAD 提交树实测**（`countLines()` 口径；探针 `.superpowers/sdd/2026-09-12-frontend-redesign-batch6-motion/tmp/t35/primitives-42.txt`，gitignored）逐值重列，**三份副本一律按本加注读**：
> - `Text` **119 / 66 / 171** · `Surface` **98 / 85 / 175** · `Button` **132 / 100 / 286** · `Modal` **260 / 81 / 296** + `Modal.exit.test.tsx` **91** · `ConfirmDialog` **202 / 58 / 297**
> - `Toast` **257 / 76 / 249** + `Toast.interrupt.test.tsx` **167** + `Toast.style.test.ts` **134** · `EmptyState` **170 / 103 / 276** · `Loading` **105 / 79 / 238** · `StatusLine` **94 / 42 / 267**
> - 共享内核 `usePresence.ts / usePresence.test.tsx / usePresence.node.test.ts / useFocusTrap.ts / useFocusTrap.test.tsx / ime.ts / ime.test.ts` = **200 / 282 / 71 / 122 / 236 / 18 / 32**
> - 接缝 / 导出面 / 守卫：`motion.css` **335** · `index.ts` **48** · `style-seams.test.ts` **299** · `style-contract.test.ts` **296** · `motion-coverage.test.ts` **186**
> - **与上表逐项漂移**：`Text.tsx` 89→**119** · `Text.css` 46→**66** · `Text.test.tsx` 130→**171** · `Surface.tsx` 97→**98** · `Surface.css` 73→**85** · `Surface.test.tsx` 168→**175** · `Button.css` 92→**100** · `Modal.tsx` 188→**260** · `ConfirmDialog.tsx` 192→**202** · `Toast.tsx` 210→**257** · `Toast.css` 65→**76** · `Toast.style.test.ts` 132→**134** · `EmptyState.tsx` 127→**170** · `EmptyState.css` 81→**103** · `motion.css` 74→**335** · `index.ts` 45→**48** · `style-seams.test.ts` 271→**299** · `style-contract.test.ts` 214→**296** · `motion-coverage.test.ts` 147→**186**（其余 23 值与上表同）。
> - **42 个文件全部 ≤300 —— 唯一例外 = `motion.css` 335**（`.css` **不在**行数门禁的 `SOURCE_EXT` 视野内；**源文件最大 = 299**）。批 6 给它追加了 6 次（T5 / T6 / T12 / T13 / T19 / T29），已越过 R11.8 的 ≤300 自律线 ⇒ 逐条登记在 `v0.22.md` 批 6 节的「诚实代价」。
> - 🔴 **口径缺口（登记，R33.3 第 2 条）**：`:258` 的机器锚点 `<!-- line-count-src: … -->`（本节与另两份各有一条）**全仓 0 个读者** —— 规格 `:835` / 豁免表 `:211` 所称「门禁按 `files=` 重测」**今日无实现** ⇒ 三副本同步**只能靠人工**。锚点**保留待将来实现**（它刻意不写数值，故它自身不会腐化）。
...
> **🔻 批 6 收口就地加注 · `pinnable` 的批 3 实际取值（R6.6；2026-09-13，**上面原文一字未改**）**：上面「批 3 全行写 `false`」**与代码不符** ⇒ **代码是权威**：`app/src/shell/columnRegistry.ts` 的 `notes-outline` 那一行逐字含 `pinnable: true`，且该文件的口径注释自称「除笔记大纲列…为 `true`」⇒ 批 3 的**实际取值 = 除 `notes-outline` 外全 `false`**（13 行逐行值：12 行 `false` / 1 行 `true`）。**本规格两处（本②与 §10「批 3 的收口」行）都写「批 3 全行 `false`」⇒ 两处原文均按本加注读**。消费方仍为 **0**（全 `app/src` 实测 15 处 `pinnable` 命中**全在同一注册表文件内**：1 处注释 + 1 处接口字段 + 13 行取值，**无任何读取点**）⇒ 本批（列折叠 Flip）**只登记消费方缺口，不接线**。
...
> **🔻 批 6 收口就地加注 · `pinnable` 的消费方终态 + 本表与 §6.3 的内部矛盾（R5.7 · R40.1 · R55/R58 的路径 B 裁决；2026-09-13，**上面原文与七条批 3 加注、批 5 加注一字未改**）**：
> - **`pinnable` 的消费方终态 = 仍为 0 处**（本批**未接线**）：全 `app/src` 实测命中仍全在同一注册表文件内（接口字段 + 13 行取值 + 口径注释），**无任何读取点**。批 6 的列折叠 Flip 走的是**授权路径 B**（`data-flip-id` 跨元素匹配、不动 DOM 结构），**不消费 `pinnable`** ⇒ 上面②「消费方仍为 0（只登记消费方缺口，不接线）」在批 6 **依然成立**，逐字登记为**未接线**。
> - 🔴 **规格内部矛盾（R40.1，两处原文均保留、就地加注消歧）**：本表 `📡 课堂 · 源列` 行写「**采集态整列隐藏**」，而 §6.3 的相变态表写「源列**保留**（采集配置面）」—— **两句互斥**。**本规格不在此裁决可见性**（模块与批 6 都未裁决）：批 6 交付的是**相位通道 + 顶栏相位 CSS + chrome 两态**，**列可见性未实现**（`app/src` 内「采集态整列隐藏」**0 处实现**；`shell/columnRegistry.ts` 的注释已就地写「两个口径都不由这里兑现」）⇒ 两个口径**按本加注并存**，实现面**以 §6.3 的「源列保留」为既成事实**（采集配置面今天不隐藏），**去向 = 批 7/8 或用户裁决**。
> - 出处：`task-34-report.md` §10（生产调用点读数）· 批 6 `rulings.md` §四十（R40.1）· §五十五/§五十八（T33 的路径 B）。
...
> **🔻 批 6 收口就地加注 · 上表的落地读数（R4.3–R4.5 · R42⑥ · R49.1④；2026-09-13，**上表原文一字未改**）**：
> - **通道**：`<html data-shell-phase>`，取值逐字 `"idle"` | `"capture"` | `"review"`；**单一写入方**终态成立 —— 全仓**唯一** `useShellPhase(...)` 调用 = `app/src/shell/phaseSource.ts:84`（`shellPhase.guard.test.ts` 的名册钉住），**载体** = `App.tsx:394` 的 `<PhaseChrome phase={phase}>`（**1 处**）。⚠️ `readShellPhase` 的 TS 读取仍 **0 生产调用点**（消费方是 CSS 的 3 条选择器）。
> - **采集态 58px LIVE 仪表 + 波形**：`--ed-nav-h-live: 58px` 落 token 真源（T17，`gen-tokens.mjs` → `tokens.css`）；链路闭合 **1 / 1 / 1** —— `<PhaseChrome>`（`App.tsx:394`）→ `shell/PhaseChrome.tsx:102` `<LiveBar …>` → `shell/LiveBar.tsx:135` `<Waveform …>`（`LiveBar` 自身 0 处直挂，由 `LiveBarHost` 之内挂载；T18 结束时的「0 调用点」已由 T19 闭合）。
> - **复习态零 chrome 的实现路径 = 相位 CSS**（`motion.css` 的相位块 + `TopBar.css` 的 `html[data-shell-phase=…]` 两态），**不是**列隐藏；**相位块的块序是硬事实**：**相位块 → 档位块 → reduced-motion 块**（实测行号 **191 < 210 < 260**）。
> - 🔴 **`navHeight.consumption.test.ts` 零改动仍绿**（T17 的自证：新增 `--ed-nav-h-live` 未破既有消费判据）。两态**像素面**（采集真的 58px / 复习真的零 chrome）**jsdom 不可判、本批未测** ⇒ 只登记（`task-34-report.md` §12）。
> - 🔴 **本表第 1 行「源列保留」与 §6.2 的「采集态整列隐藏」互斥** —— 消歧见 §6.2 的批 6 加注（**本批未实现列隐藏**，两个口径按加注并存）。
...
> - **印样数据契约（`proof` 视图；规格未给 ⇒ 批 5 T8 定，登记在此；收口评审 Minor-4 落位）**：props = `Pick<SessionViewSlot,"detail"|"imageUrl">`（**type-only**、其余 8 槽一个不接 ⇒ **结构上不可能取数/交互**；也不许自己 import `convertFileSrc`）；**DOM 锚点** = `data-testid="session-proof-view"`（根，空态分支也在）· 每张 `data-proof-sheet={first_seen_ms}` · `figure[data-proof-figure]`（图注位**恒在**，有图/无图两条分支都渲染）· `div[data-proof-body]`（正文区）；**不复用** `SessionScreenCard`（两形态只共用数据语义，代码真源是那张卡）。**七条语义（F1–F7）**：**F1 重叠谓词 = 半开区间** `seg.start_ms < last && seg.end_ms > first`（零宽屏 `first === last` **归一为点** `[first, first+1)`；**首尾相接不算重叠**；与 `useSessionDetailData` 的「屏 → OCR 块」同源）· **F2** 跨屏段在**两张印张上都出现**（重叠归属而非切分归属；与任何屏都不重叠的段**不出现在本视图** ——「原文不丢」由默认原文视图承载，见 §7.3①）· **F3** 段序 = `start_ms` 升序、并列时 `id` 升序（**先复制再排序**，不原地改 props 数组）· **F4** `<img>` 仅在 `image_ref !== null` **且** `imageUrl(ref) !== null` 时渲染（且 `imageUrl` **只在 `image_ref !== null` 时被调用** —— 计划漏了「ref 非空但解析不出」这一支，否则渲染 `src={null}` = 碎图）· **F5 两槽位锚点**（台账口径的「两槽位锚点」= 上面的 `figure[data-proof-figure]` / `div[data-proof-body]`；计划只锚了根与印张 ⇒ 没有这两个边界，P2「图注位保留」/P3「正文按区间归属」只能退化成「整篇 `textContent` 包含」的弱判据）· **F6** 序号 = `screen_id ?? index + 1`（与容器 `SessionScreenCards` 逐字同源）· **F7** `detail.screens.length === 0` ⇒ 走 `EmptyState`（`testId="session-proof-empty"`，标题「本会话无画面要点」，说明指向原文视图；**根 `data-testid` 仍在**）。**出处**：`task-8-report.md` §2.1–§2.3（逐条理由 + props 表）· `progress.md` §二十（契约原文）—— 两者都在 `.superpowers/**`（gitignored、不入库）⇒ **本行即该契约在仓库内的持久记录**。**判据侧**：`views/session/SessionProofView.test.tsx` 5 用例（P1–P5 各带变异体 M1–M8）+ 只读判据 P4 与 `nativeButton.ratchet` ①② 双红；`SessionProofView.tsx` **168 行**。
...
### 7.4 笔记「带证据三轨」—— 批 7 规格章（🔴 **本批只交规格，实现未做**）

> **出处与状态（这一行是本章的定位，别跳读）**：`docs/versions/v0.22.md:561` 逐字「**笔记「带证据三轨」→ 批 7（必须先写规格）**」，判否依据 = 批 5 `task-13-report.md` **§④**（B7-1…B7-9）+ **§②**（探针读数）。🔴 **本批只交规格，实现未做** —— 本章是**批 8 的输入契约**，不是交付证明；🔴 **全仓 `NoteEvidenceTrack*` = 0 命中**（本批独立复测：`app/src/**` 的 `.ts`/`.tsx` **PROD 0 处 / 0 文件 · TEST 0 处 / 0 文件**；仪器 `.superpowers/sdd/2026-09-12-frontend-redesign-batch7-unwired/tmp/t20/probe-spec-readings.mjs`，负控 `zzz_no_such_symbol_zzz` = 0）⇒ **不存在「第三种笔记视图已经交付」这回事**，本章也**不得**被读成它。
> - 🔻 **口径更正（2026-09-13 · 批 8 T3；上一行的**实测域**收窄，原文保留）**：上一行逐字写的「**全仓**」是**字面假命题** —— **它包含本行自身**（上一行既含「全仓」也含那个以 `Track` 结尾的符号名）⇒ **自我指涉**（与 §11-7 在 `:905-906` 的更正**同源、同形**）。🔴 **T3 于 `7c1fcc41` 实测**（入库域 = `git ls-files`，**1,615** 文件；口径 = 行含子串，剥注释抹等长空白保偏移）：**`app/src/**`（生产 + 测试）= RAW 0 行 / 剥注释 0 行 / 0 文件** ✅ · **`app/src/**` 之外 = RAW **27** 行 / 剥注释 **27** 行 / **4** 文件，**全部在 `docs/**`**（按顶层目录 = `docs:27` 行 / `docs:4` 文件）—— **含上一行自身**。
> - ⚠️ **口径注（勿与 §11-7 的读数混用）**：本注的「剥注释」**只对 `.rs` / `.ts` / `.tsx` 生效**（`.md` 无注释语义，原样比对）⇒ 本注域内两者恒等；§11-7 的 `:905` 那组读数（RAW **16** / 剥注释 **6** / **3** 文件，时点 `53bd0759`）**口径与时点都不同** ⇒ 🔴 **两组数不得互推、不得当趋势读**（§3.5「数字必须带口径与时点」）。**本注自身含该符号名恰 1 次** ⇒ 落注后全仓 RAW **+1 行 / +0 文件**（即 **28** 行 / **4** 文件），**`app/src/**` 仍 0 行 / 0 文件**。
> - ⇒ 🔴 **正确表述**：**「`app/src/**`（生产 + 测试）= 0 处 / 0 文件」**（这才是「实现未做」的**有效证据域**）；**`app/src` 之外的 27 行**全是**文档与计划里的提及**（其中含本章自身），那**不是实现**。**上一行的「全仓」一律按本注收窄读**（收窄后与 §11-7 的 `:906` 现口径一致）。阴性对照 = 本任务**现造**的随机串（**入库域实测 0 命中**；因它一旦入库即失效，故本注**不写它**，读数见批 8 `task-3-report.md`）。
> - 🔧 **T3 自捉更正（2026-09-13，落注后复测；上面三条原文保留）**：① ⚠️ 上一条「口径注」末句的**自指预测**「本注自身含该符号名恰 1 次 ⇒ 落注后全仓 RAW **+1 行**（即 **28** 行 / **4** 文件）」**实测不成立** —— 上一条**刻意用「以 `Track` 结尾的符号名」指代、并未复写该串** ⇒ 它**不含**该串，**不产生自增**。🔴 **实测（同域 = `git ls-files` · 同口径 · 落注后）**：全仓仍 **27 行 / 4 文件**，**`app/src/**` 仍 **0 行 / 0 文件** ⇒ **以本实测为准；「+1 行 / 28 行」按本注作废**（本条更正同样不复写该串 ⇒ 也不改变该计数域）。② 上一条「正确表述」里的「**上一行的「全仓」**」**指代错位** —— 应为**本章定位行 `:542`**（`:544` 是「口径注」、**不含**「全仓」）。
> - 📌 **本节 T3 各注的行号锚口径（2026-09-13 · T3；P-36 的落地）**：上面三条注（含本条）里的**行号锚一律是 `7c1fcc41` 时点的编号**（即**本注自身插入行之前**的编号）—— 🔴 **T3 的实测证据**：`7c1fcc41` 时点它写 `:905-906` / `:565`，而 `4966d9ee` 时点实测已漂到 **`:921-922`** / **`:576`**（漂移量 = 本任务在它们上方插入的 **16** / **11** 行；本章定位行 `:542` 未变）。⇒ 🔴 **跨时点引用一律按「锚名」读**：`:905-906` ⇒ **§11-7 的 T21 口径更正** · `:565`（见 §D 表下的 T3 加注）⇒ **§D 标题** · `:540` ⇒ **本章定位行**。⚠️ **行号随时点漂移（P-36），数字只作时点读数、不作锚**；本条声明本身也不再给「当前绝对值」。

#### §A 三条验收口径的改写（E1 / E2 / E3 —— 模型 / 视图层的全部判据面）

| # | 旧口径（今日规格原文） | 🔴 改写成 | 为什么必须改（类型上不可能满足旧口径） |
|---|---|---|---|
| **E1** | 「**每段**有 `data-evidence-for`」 | 「**每段**都有 `data-evidence-for`（**含无锚点段**）∧ **只有带锚点段**才渲染证据列」 | 无锚点段落客观存在（§C 的四类）⇒ 旧口径把「无锚点」与「缺陷」混为一谈；新口径把**无锚点本身**变成一等状态（有锚点无证据列 = 缺陷；无锚点无证据列 = 正常） |
| **E2** | 「`data-evidence-id` 与关联 id **逐字一致**」 | 🔴 **降级为 ms 最近邻**，且必须显式给出三件事：**① 容差**（\|锚点 ms − 候选 ms\| ≤ 容差才算命中；容差值由实现批定，**必须写进判据而非注释**）**② tie-break**（多个候选同 ms 或同距离 ⇒ 取**确定序**：`start_ms` 升序 → 同值取 `id` 升序；**不得**依赖数组顺序）**③ 失配的显式标记**（容差内无候选 ⇒ 出**显式「无证据」标记**，**不许静默取最近的一个**） | 段首锚点只带 `ms`（`[[ts:63000]]`），而证据本体是 `session_segments.id` / `session_ocr_blocks.id`（`artifact.rs:62-69`）⇒ **`ms` 在类型上无法与 `id` 逐字一致**；旧口径是一条**永远不可能满足**的断言 |
| **E3** | 「派生段落数」当分母 | 🔴 **两个分母并列、不许混用**：**分母甲 = 派生段落总数**（渲染器实际产出的段落节点数）· **分母乙 = 带锚点段落数**（其中含 `[[ts:ms]]` 锚点者）。⇒ 报告与判据**必须分别给出两个比值**（「有锚点的覆盖率」用分母乙；「全篇覆盖率」用分母甲），🔴 **禁止**只报一个数就称「覆盖率」 | 分母只有一个时，无锚点段落会被算成「未覆盖」⇒ 读数**结构性偏低**，而真实缺陷面（有锚点却无证据列）会被稀释 |

> 🔻 **批 8 T3 就地加注 · 上表 E2 行的**出处**更正（2026-09-13；**上表原文与各格一字未改**）**：E2 行的「为什么必须改」列逐字用 `artifact.rs:62-69` 支撑「证据本体 = `session_segments.id` / `session_ocr_blocks.id`」。
...
| # | 类 | 出处（发射路径） | fixture 必须断言什么 |
|---|---|---|---|
| 1 | **`OcrDirect`**（图文会话） | `note_filter_render.rs:85`（走 `note_filter_ocr::rebuild_ocr_markdown`；该文件里 `format_timestamp` 调用点 = **0 处**） | 段落**全部无锚点** ⇒ 每段仍有 `data-evidence-for`、**0 个证据列**、**0 个「无证据」告警**（无锚点 ≠ 异常） |
| 2 | **`Web`**（网页正文） | `note_body_source.rs:24-27`（`BodySource::Web` 是 `#[allow(dead_code)]` 预留变体，正文直取 `web_session_pages.markdown`） | 同上；**且**这条 fixture 是「未接线变体」的**唯一**覆盖面（它今天没有生产发射路径） |
| 3 | **手动笔记** | **无发射路径**（用户手写/改写） | 段落无锚点 ⇒ 同 1；**另断言**：用户编辑**不得**被自动补锚点（§4.4 的「绝不猜」同源纪律） |
| 4 | 🔴 **`anchor_timestamps=false`**（**关闭开关的用例必须有**） | `purify_config.rs:66,96`（**默认 `true`**）；金测试 `note_filter_golden_tests.rs:125-127` 已逐字钉住「关闭后无锚点」 | **同一份输入**在开关 `true` / `false` 两态下各跑一次：`true` ⇒ 有锚点段 > 0 且证据列只在那些段上；`false` ⇒ **锚点 0 个**、证据列 0 个、`data-evidence-for` 仍逐段在。🔴 **只测默认配置 = 假绿**（会让实现退化成「只在默认配置下绿」） |

#### §C 契约：精修前后两态（「精修后段落锚点丢失」是**有意**，不是 bug）

- **事实**：`anchor_strip.rs:8-16` **主动剥离**段落锚点；消费点 = `ai_refine_task.rs:155` · `commands_ai_refine/workbench.rs:81` · `commands_ai_refine/session.rs:57`；剥离后**只回挂章节锚点**（`ai_refine_protocol.rs:46`）。
- 🔴 **写成契约（本节的交付物）**：**精修产物的段落级锚点不保证存在**；精修**输入**路径上「段落锚点消失」是**设计行为**（防止模型把 in-band 锚点当正文改写/编造）。⇒ 带证据三轨在**精修后**的段落上必须走 §A 的 E2 失配分支（显式「无证据」标记），**不得**把它报成缺陷或回滚精修。
- **两态 fixture 清单**：① **精修前**（`notes.content` 原文，锚点在段首）② **精修后**（章节锚点回挂、段落锚点 0）。两态各一份 fixture，**同一套断言**（E1 逐段锚 + E2 三件 + E3 双分母），差值必须**逐条归因到本节契约**。

#### §D 数据路径与红线（**需要控制方另裁，不在执行单元权限内**）

1. **段落无稳定 id 的根因**：`notes.content` 是扁平 `TEXT`（`db_migrations.rs:18-27`），段落身份由 `noteCardModel` **派生** ⇒ 派生序号**会随编辑漂移**（用户在上方插入一段 ⇒ 后面所有段落身份全变）。
2. **要稳定配对的两条路**：**(a) 派生序号**（零 schema 改动，但漂移可观测）· **(b) 新增段级身份** ⇒ 🔴 **触及规格 §3 红线 6「后端数据模型零改动」** ⇒ **必须控制方另裁**，本批**不开**。
3. **唯一段级引用**：`artifact_blocks.refs_json.segment_id`（`artifact.rs:64`）—— 维度是 **session 而非 note**，且 **0 个 `#[tauri::command]` 返回该类型**（0 IPC 出口）⇒ 若要用**必须新开 command**，同 2 的红线裁决。🔴 **不得**在执行单元内顺手开。
4. 🔴 **不得新写第 3 支手写 markdown 解析器**（与 §7 的 markdown 归一冲突）；实现若落地，**复用** `utils/markdownLine.ts` / `NoteMarkdown` 的既有渲染链。

> 🔻 **批 8 T3 就地加注 · 本节 §D 的两条红线裁决（2026-09-13；**§D 的标题与第 1–4 条的原文一字未改**）**：第 2 / 3 条逐字写的「**必须控制方另裁**」（标题 `:565` 同写「**需要控制方另裁，不在执行单元权限内**」）**已由控制方裁决**（控制方 §2 **A2** 逐字）：
> - 🔴 **走 (a) 派生序号** —— 即第 2 条的 **(a)**：**零 schema 改动**；§D1 的漂移事实**不变**（漂移**可观测**，按其登记，**不因此改设计**）。
> - 🔴 **明确不开 (b)「新增段级身份」**（第 2 条 (b)，仍触 §3 红线 6）**，也不开 §D3 的「新开 command」** —— 前端取数走既有 `get_session_detail`（出处与实测见 §A 表下的 T3 加注）⇒ 第 3 条末句「若要用**必须新开 command**」**按本注不再成立**（**不开** command 就够）。
> - ⇒ **本节标题与第 2 / 3 条不再是「待裁」状态**；下游实现单元**不需要**任何新增 IPC / schema。§E 逐条登记的「批 8 输入 ②（§D 的两条裁决）」**按本注闭合**。
> - 📌 **锚口径（承接 §7.4 定位行下 T3 的同类声明；2026-09-13 · T3）**：本注里的「标题 `:565`」是 **`7c1fcc41` 时点**的编号 ⇒ 🔴 **一律按锚名「§D 标题」读**（本任务在其上方插入行后已漂移；绝对值见上引声明）。

#### §E 批 8 输入（逐条登记）+ 诚实边界

- **批 8 输入**：① 本章的实现（视图 + ms 最近邻纯函数 + 四类 fixture）② §D 的两条裁决（段级身份 / 新 command）③ E2 的**容差取值**（本章只定「必须有」与「三件形态」，**不定数值**）。
- **诚实边界**：本章**没有任何实现**，因此**没有**可跑的判据、**没有**覆盖率读数、**没有**观感结论；`NoteEvidenceTrack*` 的 0 命中是**本章未实现的证据**，**不是**「已交付但没测」。
...
> **🔻 批 6 收口就地加注 · §8 的实现面回填（§8.1–§8.7 逐小节；2026-09-13，**本节所有原文与批 5 加注一字未改**）**：
> - **§8.1 四层纲领的逐层落点**：响应层 = `motion.css` 的**元素级**全局回执 + 原语层四类回执（T12，**零调用点改动**）· 环境层 = `motion.css` 的环境层四件 + `app/src/motion/env.ts`（幅度真源，T13）· 编排层 = `useGSAP()` + 六个签名 hook（波 C）· 生长层 = 本批**只做「到期刻度生长」**（`components/review/useScaleGrowth.ts`），其余登记。
> - **§8.2 引擎**：`gsap@3.15.0` + `@gsap/react@2.1.2`（**本批唯一一次「零新增依赖」破例**，R2.1）；**唯一入口** = `app/src/motion/engine.ts`，顶层 `gsap.registerPlugin(useGSAP, Flip, ScrollToPlugin, CustomEase)`（**恰 4 个、顺序逐字**）；🔴 **双实例陷阱**（测试底座独有）：`@gsap/react@2.1.2` 无 `exports`、`main` 指 CJS ⇒ Vitest external 后拿到**另一份 gsap 运行时**，不 `registerPlugin` 时 `context.revert()` **静默空转**（实测：裸 `useEffect` = 2 tween、`useGSAP()` **也是 2 个**；注册后 = **1 个**、卸载后 `transform === ""`）。✅ **真实产物上成立**：`vendor-gsap-BQLVn3Z6.js`（**105,584 B**）**在产物里、不在 `firstScreen.chunks`**（首屏仍 3 chunk）。
> - **§8.3 双基调**：载体 `data-tone`（取值逐字 `"instrument"` | `"paper"`）+ 两个缓动 token（`gen-tokens.mjs` 真源）+ `CustomEase` 具名 ease。⚠️ **量化事实**：CSS 侧 `cubic-bezier` 只是近似 ⇒ **`maxDiff ≈ 0.409 @ x=0.40`**（本批**接受该差**，**但接受 ≠ 可两路横跳**：同一落点只许一条路径）。
> - **§8.4 token 与清单**：10 个变量已迁入 `gen-tokens.mjs` **单一真源**（`motion.css` 的 `--ed-*` 定义**恒 0**，由 `style-seams.test.ts` 的回归封条钉住）；**「不 animate height」已判据化**（T19 的 V1：相变 chrome 走绝对定位 + 交叉淡入）。
> - **§8.5 三档**：载体 `data-motion`（`"eco"` | `"standard"` | `"rich"`，默认 `standard`；持久化键 `motion:intensity`；初值跟随系统）；**源序 = 防御性钦定**（真实承重机理 = **reduced-motion 块逐条带 `!important`**；🔴 **不得**给档位块加 `!important`）；三档的机器面 = 档位映射 + DOM 属性 + 源序 + 时长参数，**观感面进「只能登记」清单**。
> - 🔴 **§8.6 六个签名动效逐条兑现度**（规格 §11-10 的四行判据「持有 / 中断 / 降级 / 三档」各有专属变异体；生产调用点读数出处 = `task-34-report.md` §10）：
>
>   | # | 名称 | 生产调用点 | 落点（逐字） | 兑现度（未兑现的带原因） |
>   |---|---|---:|---|---|
>   | 1 | 对齐 | **1** | `views/session/SessionTriTrackView.tsx:251` | ✅ 可判（三轨共轴 + 同 ms 高亮 + 位移 ≤8px）；**真实数据上「同 ms」极少成立** ⇒ 共轴容差 = 规格未给数字 ⇒ **不发明**，去向产品裁决 |
>   | 2 | 显影编排 | **1** | `components/session-detail/SessionRawView.tsx:96` | 🟡 **部分**：编排可判；节奏 = `charRate` **字符率近似**（**不得**称为「真实语速」—— 规格未定义语速函数）；🔴 **反向 / 重播 `replay()` 入口 = 0**（状态机与判据齐备、无生产入口） |
>   | 3 | 相变凝固 | **1** | `shell/LiveBar.tsx:90` | ✅ 可判（波形收束成直线 / 琥珀退去 / 导航淡入三件 + 可反向） |
>   | 4 | 刻度生长 | **1** | `components/review/ReviewSessionPanel.tsx:98` | ✅ 可判（生长 / 回缩；回缩用 `review_card` 的**精确** `intervalDays`） |
>   | 5 | 时间码回跳 | **1** | `views/session/TimeRail.tsx:140` | 🟡 **部分**：**落位 + ≤8px 掠过**已交付；**长距离连续滑动未交付** |
>   | 6 | 记忆浮现 | **1** | `components/review/ReviewSessionPanel.tsx:102` | ✅ 可判（墨色洇开 + 字距用 **`x` 而非 `letterSpacing`** + 剪报底纹左刷 + 评分按钮浮起） |
>   | （附）| 列折叠 Flip | **2 / 8 处** | `components/ChatSidebar.tsx:80` · `pages/GoalsPage.tsx:42` | 🟡 走**授权路径 B**（`data-flip-id` 跨元素匹配、**零既有断言改动**）；其余 **6 处未接线**；🔴 `Flip` 会写 `width`/`height`（layout 属性）⇒ 与 §11-10 的属性集合判据**天然冲突**，如实登记（**不为让判据变绿而放宽 R8.4**） |
>
> - 🔴 **`[[ts:ms]]` 端到端只通一半**：`NoteMarkdown` 的芯片已带上毫秒（T26 **修掉「芯片是死的」这一真实缺陷**），但**会话页那一跳未交付** ⇒ 登记为**批 7 首要候选**（`task-26-report.md` 的逐行成本与 patch 落点保留）。
> - **§8.7 三条不做**：**未触碰**（`bounce` / `elastic` / 粒子 / 页面转场滑动 = 0 处）。
> - 🔴 **像素面全部未测**：本节的观感面（真的直线 / 真的 0.72 / 真的 220ms 淡入 / 真的 58px）**jsdom 不可判** ⇒ 只登记，见 `v0.22.md` 批 6 节的「诚实代价」。
...
> **🔻 批 6 收口就地加注 · `pinnable` 的批 3 实际取值（R6.6；2026-09-13，**上格原文一字未改**）**：上格「批 3 全行 `false`，无消费方」的**取值半句与代码不符** ⇒ **代码是权威**：`app/src/shell/columnRegistry.ts` 的 `notes-outline` 那一行逐字含 `pinnable: true`，且该文件的口径注释自称「除笔记大纲列…为 `true`」⇒ 批 3 的**实际取值 = 除 `notes-outline` 外全 `false`**（13 行逐行值：12 行 `false` / 1 行 `true`）。**本规格两处（本行与 §6.2 的批 3 回写②）都写「批 3 全行 `false`」⇒ 两处原文均按本加注读**；「无消费方」半句**成立**（全 `app/src` 实测 15 处 `pinnable` 命中全在同一注册表文件内，**无任何读取点**）⇒ 本批（列折叠 Flip）**只登记消费方缺口，不接线**。
...
> **🔻 批 8 终态就地加注（2026-09-13 · T21；上格与上面各 `>` 块的原文一字未改）** —— 上表 **批 8 行**（`| **8 治理收口** | 豁免表终态 · 回写 ui-ux-system.md / theme.md · 新增动效规范章节 · 需求池同步 | 11 条验收全达标 |`）有两处**不得照字面读**：
> - 🔴 **「11 条验收全达标」= §11 的 11 条整册验收**，**不是**批 8 的行内验收。批 8 计划 `## Global Constraints 二` 逐字写死：「**本批的「三条验收」= 三条主线的验收**……**不是**规格 §11 的 11 条整册验收 —— 那 11 条**逐条登记去向**」⇒ 🔴 **批 8 只兑现与三条主线直接相关的部分**，**不得**声称「11 条全达标」。
> - 🔴 **「新增动效规范章节」已在批 6 落地**（`docs/standards/motion.md`，`bffed928`）⇒ 留批 8 的是 `docs/product/ui-ux-system.md` / `theme.md` 的**四层动效 / 三档强度回写**，🔴 **而批 8 仍未做**（它只回写了「审校模式」那一节：`b077d530` · `ffb7e28a`）⇒ **登记为下一批输入**。
> - **批 8 的三条主线兑现度（逐条机器读数；出处 = [`docs/versions/v0.22.md`](../../versions/v0.22.md) 的「批 8 · 治理收口与三条主线」节）**：
>   · **主线一**（「带证据三轨」+「审校模式」两条能力）✅ **成立** —— 注册表第三项落地：`app/src/views/registry.ts` 的 `FROZEN_VIEW_KEYS.note` **由 2 键变 3 键** = `["raw", "cardflow", "evidence"]`（T21 于 2026-09-13 逐行直读）；全量 vitest **235 文件 / 2300 passed / 0 failed / 0 pending**（T21 亲测，时点 **2026-09-13T17:34+08:00**）。
>   · **主线二**（观感/像素面「收编，不立项」）✅ **成立（含「有读数 ≠ 已统一」的限定）** —— 仪器 `scripts/viewport-probe.mjs` **298 行**在盘、按需入口 `app/package.json` 的 `check:visual` 在盘、仪器自检 **9/9 ✅**、**14 条观感欠账逐条有读数或具名归属**。
>   · **主线三**（治理余项）✅ **四条 4/4 落地** —— **CI 挂载**（`pr-check.yml` 的无条件 job + 散文探针单列 step）· **`scripts/validate-all.mjs` 已删** · **`SURFACE_TAG_REGISTRY` 的键改为 `(file, tier)`** · **`subject ≤50` 已装上**（`commitlint.config.js`）。
> - 🔴 **段 8f 的四项用户裁决（U1–U4）另计，不并入三条主线**：**U1 = 音频两条都修** ✅（配置通道 + 面板真开关 + 删会话级联清音频；命令注册表 **311 → 312**，依据**用户裁决 A**）· **U2 = 只移除凭据槽的代码路径** ✅（**已存密钥数据保留、应用内不可达**）· **U3 = `docs/tech-debt/` 归档到 `docs/archive/2026-09-13/`** ✅ · **U4 = 观感按需入口入库 + 触发条件写死** ✅（**不接 husky、不进 CI**）。🔴 **U5（真机 / WebView2 冒烟）沿用「跳过」** ⇒ **7 条真机类继续登记、不假装完成**。
> - ⚠️ **两条口径注**：① **本批不新增门禁成员**（八闸集合不变；`check-exemption-prose` 只以 **CI step** 转正、**不进八闸对账**）② 🔴 **批 8 的提交留在本地、未 push**（用户前提；「不 push」是本批的唯一强制约束）。
> - 🔴 **本节的自引用边界**：上格那「批 8 行」所在的本文件**没有**在批 8 内被改写成「已交付」—— 本注是**追加**，`−` 列为 **0**（承 [`docs/standards/documentation.md`](../../standards/documentation.md) 第四部分之补）。
...
5. `prefers-reduced-motion` 覆盖率 **100%**，且系统优先于强度档位。
   **判据（批 0-D 拍定）= 基类名单 + 动画落点名单，不是「全类集合 ⊇」**：每一类原语的**根类**（动效挂在它身上的选择器）必须在
   ...
   > **🔻 批 6 收口就地加注 · 本条在批 6 的机器判据与读数（R11.3 / R12.4 / R40.2；2026-09-13，**上面原文一字未改**）**：
   > - **判据落点**：`app/src/ui/primitives/motion-coverage.test.ts`（名单 + **源序** describe）+ `app/src/motion/responseSeams.test.ts`（I-2 逐条声明带 `!important` / I-3 两块之间零规则块）+ `app/src/motion/responseCoverage.test.ts`（响应层七类动作回执）+ `app/src/motion/env.test.ts` + `app/src/motion/shift.guard.test.ts`。✅ 终态（T34 逐件复跑）：`motion-coverage` **9/9 passed** · `style-seams` **18/18 passed**。
   > - 🔴 **已知残洞两条（如实登记，R46② / R47⑤）**：① `TopBar.css` 的**动画落点仍无「必须在 reduced-motion 名单里」判据**（结构性缺口只补到扫描器这一层；`TopBar.test.tsx` **余 2 行** ⇒ 任何追加**必须先拆件**）· ② reduced-motion 名单**今天只能容纳「基类 或 有 `animation` 声明的选择器」**，**无法枚举 transition-only 落点** ⇒ `Waveform.tsx` 的行内 transition 接缝与 `ColumnResizer` 的余量同源（**机制性缺口，不是「忘了列」**）。
   > - **环境层四件的兑现度**见 §4.3 的批 6 加注（第 ③ 件 = 1 处调用点，要求 ≥2）；「落了 CSS seam」**不算**已交付（R11.3 逐字）。
6. 会话与笔记各 **≥2 种**展示形式可用，原文形态不丢。
   ...
   > ↳ **批 7 收口就地加注（C9.17 / C9.10；2026-09-13，**本条第 6 行的原文与批 5 加注一字未改**）—— 第三种的**规格**已在 **§7.4** 交付，**实现仍未做**：**
   > - 🔴 **逐字状态：本批只交规格，实现未做。**「带证据三轨」的验收口径改写（E1/E2/E3）、两个分母、无锚点四类 fixture 清单、精修前后两态契约**全部写在 §7.4**；**代码零行**。
   > - 🔴 **本条验收仍按批 5 的读数成立**（**2 ≥ 2**）：笔记视图键终态 = `["raw","cardflow"]`（`app/src/views/registry.ts` 的 `FROZEN_VIEW_KEYS.note`），**本节不改变这个数**。
   > - **实测（批 7 独立复测）**：全仓 `NoteEvidenceTrack*` = **0 处 / 0 文件**（PROD 与 TEST 双侧；负控 `zzz_no_such_symbol_zzz` = 0）⇒ **不得**读成「已交付」。
   > - **批 8 输入**：§7.4 的实现 + 它的两条红线裁决（段级身份 / 新 command）。
   > - 🔻 **口径更正（2026-09-13 · T21 补轮；上一行的**实测域**收窄，原文保留）**：上一行逐字写的「**全仓**」是**字面假命题** —— 它**包含本行自身**（本行就含该串）⇒ **自我指涉**。🔴 **T21 于 `53bd0759` 实测**（入库域 = `git ls-files`，**1,614** 文件；口径 = 行含子串，剥注释抹等长空白保偏移）：**`app/src/**`（生产 + 测试）= RAW 0 行 / 剥注释 0 行 / 0 文件** ✅ · **`app/src/**` 之外 = RAW **16** 行 / 剥注释 **6** 行 / **3** 文件，**全部在 `docs/**`**（按顶层目录 `docs:16`）—— **含本行自身**。
   > - ⇒ 🔴 **正确表述**：**「`app/src/**`（生产 + 测试）= 0 处 / 0 文件」**（这才是「实现未做」的**有效证据域**）；**`app/src` 之外仍有 16 行 / 剥注释 6 行的文档提及**，那**不是实现**。**上句的「全仓」一律按本注收窄读**（本页 §7.4 的 `:538` 早已按 `app/src/**` 写，**两处现口径一致**）。
   > - ⚠️ **探针卫生教训（本轮实测复现）**：该声明的**阴性对照** `zzz_no_such_symbol_zzz` 在**入库域**命中 **7 行 / 5 行（剥注释）/ 3 文件（全在 `docs/**`）** ⇒ 🔴 **对照串被自己的产物污染**（它被写进了报告与文档）⇒ **阴性对照一旦入库就不再有效**；改用**每次现造的随机串**，或**把对照域明确限定在 `app/src/**`**。（阳性对照 `useTriTrackAlign` 在 `app/src` = **17 行 / 12 行（剥注释）/ 6 文件** ⇒ 非假 0。）
> **🔻 批 8 终态就地加注 · 本条上面那句「批 8 输入」已闭合并给终态读数（2026-09-13 · T21；上面各块原文一字未改）**：
> - 🔴 **第三种「带证据三轨」已于批 8 交付**：模型层 `app/src/views/note/noteEvidenceModel.ts`（**202 行**）+ 视图片 `app/src/views/note/NoteEvidenceTrackView.tsx`（**178 行**）+ 容器取数 `app/src/hooks/useNoteEvidence.ts`（**157 行**），提交 = `de1f43aa` · `8cc627de` · `08b1afda` · `e493eac8`（行数为 T21 于 2026-09-13 用 `countLines()` 口径实测）。
> - 🔴 **笔记视图键由 2 键变 3 键**：`app/src/views/registry.ts` 的 `FROZEN_VIEW_KEYS.note` 逐字 = `["raw", "cardflow", "evidence"]`（T21 逐行直读）。
> - 🔴 **但本条验收的口径不变，仍是「各 ≥2 种」** ⇒ 会话 **5** / 笔记 **3**，`3 ≥ 2` ✅；**不得**把「终态 3」写成「本条要求 3」或「本条因批 8 而改变」。
> - **两条红线裁决的落地（承 §7.4 §D 的批 8 加注）**：① **段级身份走「派生序号」**（**零 schema 改动**）② **不开新 command**（复用既有 `get_session_detail` 已返回的 `segments` + `ocr_blocks`）。
> - 🔴 **仍未做 / 未覆盖**：**真机 / WebView2 冒烟由用户裁决跳过（U5）** ⇒ 本条实现面的「端到端」**止于 jsdom / Rust 单测层**；**像素 / 观感一致性未验证**（`≥2` 是形式数，不是「两种形式看起来对」）。
...
> **🔻 批 8 终态就地加注 · 本条在批 8 的状态（2026-09-13 · T21；本行原文一字未改）**：
> - 🔴 **批 8 对本条零改动、零复跑**：批 8 的三条主线（两条能力 / 观感收编 / 治理余项）与四项用户裁决（U1 音频两条 · U2 凭据槽代码路径 · U3 `docs/tech-debt/` 归档 · U4 观感按需入口）**没有一条落在这条验收上** ⇒ 🔴 **本条的结论仍以批 7 的收口读数为准**，**不得**读成「批 8 重新验证过」。
> - 🔴 **防误读（逐字）**：批 8 的 **U1 是「音频存储开关 + 删会话删音频」**，**与本条的「标签 / 档位」无关** ⇒ **不得**把它算进这一条；批 8 也**未触碰**任何标签线或档位通道的生产代码。
> - 🔴 **本条的「真机面」仍未覆盖**：批 7 的「档位选完真生效」判据是**代理判据**（记忆体往返 + 负控），**不是**「采样参数真被采集链路消费」的像素级证据；且**真机 / WebView2 冒烟由用户裁决跳过（U5）**。
> - **批 8 的相关读数（只作旁证，不作本条判据）**：`check-command-registry` **312 / 312 / 0** · `cargo test --test app_lib_tests` **2378 passed / 0 failed / 6 ignored** · `npx tsc --noEmit` **0 错**（均为 T21 亲测，时点 2026-09-13T17:32–17:35+08:00）。
...
9. JS 包首屏 gzip 达标或给出瓶颈清单；GSAP 只在独立 chunk 懒加载。
   ...
   > **🔻 批 6 收口就地加注 · 本条后半（GSAP 懒加载）在批 6 的落地读数（R2.1 · R11.1 · R63.3；2026-09-13，**上面原文与批 2/批 5 进度注一字未改**）**：
   > - ✅ **前半继续达标**：首屏 JS gzip **103.60 kB < 200 kB**（**余量 96.40 kB**），`scripts/check-bundle-budget.mjs`（**真实构建**，取锁 `attempt=1` + `finally` 释放）**exit 0**。**Δ（对批 5 收口的 100.49 kB）= +3,118 B（+3.10%）**；**原始 +8,382 B（+2.63%）全部落在入口 chunk**（108,620 → **117,002 B**）；`vendor-react` / `vendor-tauri` **逐字节不变**；**首屏 chunk 数 3 → 3（Δ 0，构造性成立）**。**CSS 68.78 kB 原始 / 15.62 kB gzip（4 个）不计入判据、只报告**（批 5 = 63.23 kB / 3 个 ⇒ **+5,549 B**；**归因必须含 T12 / T13 / T19 / T29**，不止 T5/T6）。
   > - ✅ **后半「GSAP 只在独立 chunk 懒加载」成立**：产物有 `vendor-gsap-BQLVn3Z6.js`（**105,584 B**）**且 `firstScreen.chunks` 不含它**；判据 = `engine.guard.test.ts` 的**闭包交集**（不是白名单），**真实构建后复跑 11/11 passed、exit 0**。
   > - 🔴 **结论口径唯一（R63.3 钉死）**：首屏静态可达的源文件数**只许用 TS-API 真实边口径** = **66 → 75（+9）**、npm 包 **4 → 4（Δ 0）**；`bundle-eager-graph` 的工具读数（91 → **103** 文件 / 4 → **7** 包）**只作旁证**，且必须注明「其中 **2 个是纯类型边**（`gsap` / `@gsap/react`，由 `import type … from "../motion/engine"` 顺边走到值导入）+ **1 个是注释字面量假命中**（`views`，出自 `app/src/types/session.ts:347` 的 JSDoc）」。
   > - **懒侧账（不计入预算，只报告）**：懒 chunk **31 → 36**、原始 **+354,344 B**、gzip **583.98 → 698.86 kB（+114.88 kB）**；归因 **94.1% / 92.0% 来自两块** —— `vendor-katex` **+227,858 B**（T28 让 `structuredBlocks.ts` 首次进图 ⇒ **`katex@0.18.4` 的第二份副本**进产物；**懒侧今天没有任何预算门禁**）+ 新块 `vendor-gsap` **+105,584 B**。🔴 该代价逐字进 `v0.22` 批 6 节的「诚实代价」，并登记两条**批 7/8 候选**：「把 `lowConfidenceClass` 从 `structuredBlocks.ts` 析出轻模块」+「**懒侧字节纳入门禁**」。
10. 6 个签名动效各自**可中断、可反向**，reduced-motion 下正确降级。
   > **🔻 批 6 收口就地加注 · 本条 6 个签名动效的四行判据落点（R8.1–R8.6 · R11.4；2026-09-13，**上面原文一字未改**）**：
   > - **「可中断 / 可反向」的判据形态 = 双断言**（**同时**断 `gsap.globalTimeline.getChildren().length`（或旧 tween 的 `totalTime()` 冻结）**与**目标元素 `style.transform`）—— 只看 `style.transform` **会假绿**（GSAP 3 默认 `overwrite:false`）。唯一出口 = `app/src/motion/controls.ts`（`interrupt()` 真 `kill`，且 `killTweensOf` **先**、`timeline.kill()` **后**）。⚠️ `getChildren()` **默认是递归口径**（本实现下的正确值是 **2**）；只有 `getChildren(false, true, true)` 才 `=== 1`。
   > - **四行判据的落点**（逐条在 `task-34-report.md` §10 给出生产调用点）：#1 对齐 `views/session/useTriTrackAlign.ts` · #2 显影 `components/session-detail/useRevealChoreography.ts` · #3 相变凝固 `shell/usePhaseFreeze.ts` · #4 刻度生长 `components/review/useScaleGrowth.ts` · #5 回跳 `views/session/usePlayheadJump.ts` · #6 记忆浮现 `components/review/useRevealMemory.ts`（每条各带专属变异体与「三档 / reduced-motion 降级」用例）。
   > - 🔴 **降级路径的诚实边界**：`prefers-reduced-motion` 下**跳终态**由 `controls.ts` 统一实现（`matchMedia` 自带守卫）；**像素面（真的直线 / 真的 220ms 淡入）不可判**；`Flip` 的几何位移**零可观测** ⇒ 只用「结构契约 + Flip 已注册 + timeline 存在 + 属性集合 ⊆ transform 族」的弱判据。
11. `npx tsc --noEmit` 0 错 · `npx vitest run` 全绿 · `cargo test` 全绿（每批）。
    ...
    > ↳ **收口后处置（2026-09-12，`cd85e4c3`；控制方已裁）**：该 flake 的状态记为「**已加固、未复现**」—— 收口修复单元做了**最小稳定性修改**（`components/KnowledgeGraphView.test.tsx` 的 **18 处 `waitFor` 超时 `5000 → 15000`**，与 `vitest.config.ts` 的 `testTimeout: 15000` 对齐；**未放宽任何断言**、未加睡眠、未改配置、未加 `retry`），但**本机两种负载各 8 次 + 无负载 6 次均未复现** ⇒ **不声称「已修好」**。**纪律**：① **变异体实验不得与全量测试并发**（负载会把 flake 变成假红）；② 今后若在**顺序执行**下再红 ⇒ 按**新 flake** 重新定位。
     ...
     > **进度（批 6 收口，2026-09-13）**：八门禁终态在 `HEAD = 975b3285` 上采集（真实构建产出 `app/dist` mtime **2026-09-13T05:10:24.838+08:00**，入口 `index-CBZzLV5U.js`）：`line-limits --full` **`>600` 0 · 301–600 档 122 · 登记条目 122**（零新增登记）· `docs-check` exit 0（扫描 **280** / 检查 **180**，五项全 ✅）· registry **313/313/0** · `tsc --noEmit` **0 错** · vitest **221 文件 / 2132 用例 / 0 失败 / 0 skip**（**一次通过、零 flake**；对拍批 5 收口的 184 / 1770 ⇒ **+37 文件 / +362 用例**）· `check-bundle-budget.mjs` **exit 0 · 首屏 103.60 kB gzip** · `bundle-eager-graph` **103 文件（源 89 + CSS 14）/ TS-API 真实边 75** · 🔴 **`cargo test --test app_lib_tests` 真跑**（本批有 Rust 改动 ⇒ **必跑**）：`running 2335 tests` → **`2329 passed; 0 failed; 6 ignored`**，**exit 0**（与批 3 基线 / 批 5 持平 ⇒ **用例数 Δ = 0**）· `cargo clippy --all-targets`：error **0** · lib warnings **15 = 基线**。
     > ⚠️ **Rust 侧的判据口径（R25.1）**：**不得**要求逐字复现 `2303/0/6` —— `ffmpeg::tests::run_captured_handles_large_output` **有基线树复现**（含 10s 墙钟超时，与全量并发时被拖超时）⇒ 判据 = 「**本批相关用例全绿 + 唯一红若是该 flake 则须单跑复绿**」。🔴 **六个负载敏感 flake 全列**（本批登记，判据 = 单跑复绿）：`components/KnowledgeGraphView.test.tsx` · `components/notes/NotesReadingColumn.views.test.tsx` · `components/ChatSaveNoteDialog.test.tsx` · `views/registryResolution.test.ts` · **`components/KnowledgeSampleView.test.tsx`（第 6 个候选）** · `ffmpeg::tests::run_captured_handles_large_output`。⚠️ 但**「并行窗口内的全量偶发红」是一类现象、不是固定名单**（评审另实测出 3 个不在名单内的瞬时红：`pages/NotesPage.test.tsx` 编辑标题→完成 · `pages/ReviewPage.test.tsx` 打开即见到期数 · `components/SessionDetailPanel.test.tsx` P1 默认视图是原文）⇒ **判据 = 「单跑复绿 + 提交树全量绿」，不是「命中某个名单」**；且**全量 vitest 必须在无并发单元时跑**（R40 附）。
...
> - **本批新增的登记项（逐条带去向）**：① **`aligned` 的残余偏差 D3–D6**（每次暂停 ≤300ms · 停止尾块 ≤200ms · 逐包重采样亚样本；**量级未实测、漂移分布今天不存在**）→ **批 7/8** · ② **导入会话无音频承载面**（音轨在 `%TEMP%` 且导入结束即删、`sessions` 表无源视频路径列 ⇒ `session_audio_path` 恒 `Ok(None)`）→ **产品裁决** · ③ **跨窗口档位 / 相位同步**（`?float=1` / `?overlay=1` 各自独立）→ **批 7/8** · ④ **孤儿 sidecar**（`{id}.wav.meta.json` 不在 `cleanup` 的 `.wav` 过滤面内）+ **删会话不删音频** → **与 ② 一并裁决** · ⑤ 🔴 **`Flip` 的 `width`/`height` 与 §11-10 属性集合判据的冲突**（Flip 会写 layout 属性）⇒ **如实登记为「属性集合判据在 Flip 上不成立」**，**不得**为让判据变绿而放宽 R8.4 → **批 7/8 与判据纪律** · ⑥ **`ADR-019` 缺号**（**不补、不复用，只登记**）· ⑦ **`docs/tech-debt/`** 仍挂起 → **用户裁决**（T36 收口报告列三个选项） · ⑧ **懒侧字节无预算门禁** + **`katex@0.18.4` 第二份副本**（R63.1）→ **批 7/8 候选**：把 `lowConfidenceClass` 从 `structuredBlocks.ts` 析出轻模块 / 懒侧纳入门禁 · ⑨ **`[[ts:ms]]` 的会话页那一跳未交付** → **批 7 首要候选** · ⑩ **`usePlayheadJump` 等 hook 无 unmount 清理**（时间线在卸载后仍存活，潜在残留）→ 波 D 已评估、**登记批 7/8**。

---

> **🔻 批 8 终态就地加注 · §12 表内「旧单 provider 凭据槽 `"default"` 在应用内永久不可撤销」那一行（2026-09-13 · T21 落库时该行在 `:979`）** —— ⚠️ **表内不能插块引用（会截断表格）** ⇒ 按批 7 §9 的先例**整表之后就地在注、并逐字点名该行**；**该行原文一字未改**（见上表）。
> - 🔴 **原文那句「读兜底与启动迁移仍在」现在是假话**：批 8 的 **T25** 已**移除那条读兜底与启动迁移路径** —— 提交 **`a054ec73`**「fix(ai): 移除默认凭据槽作为合法槽位的路径」（parent `9b1e8aa8`；写集恰 **5 条** = `ai_credentials.rs` · `ai_credentials_tests.rs` · `app_setup.rs` · `commands_ai_providers.rs` · `commands_ai_providers_tests.rs`），紧随其后的是 **`ae3bba3e`**「test(ai): 收紧遗留凭据槽判据的具名红点」（parent = `a054ec73`）。
>   ⇒ 🔴 **原文保留、本注为准**：上表该行的「**读兜底与启动迁移仍在**，前端本就 0 引用（非回归）」**描述的是批 1 时点**；**批 8 时点已不成立**。
> - 🔴 **必须并读的另一半（否则会把「移除了代码路径」误读成「数据被抹除」）**：**已存密钥数据保留，且应用内不可达** —— 这是 **U2 的 (ii)「不做」的刻意边界**（**无迁移即销毁不可逆**）⇒ **本批只移除代码路径，不物理抹除任何已存数据**；**将来若要清理需新的用户裁决**（登记为下一批输入）。
> - ⚠️ **同源文本另有两处、按「时态判据」不动的**：`docs/superpowers/plans/2026-09-11-frontend-redesign-batch1-deletions.md` 与 `docs/superpowers/plans/2026-09-13-frontend-redesign-batch8-groundwork.md` 各一处「读兜底与启动迁移仍在」⇒ **历史计划文本、一字不动**。
> - ⚠️ **真实盲区（如实登记，不掩饰）**：**启动期行为无单测**（该判据在行为侧不可达）⇒ T25 的那条红点由**静态判据**承担；且 **「仅有 `default` 槽的真机用户凭据会失效（需重填）」这一风险在本环境测不了**（**真机 / WebView2 冒烟由用户裁决跳过，U5**）。
> - ⚠️ **引述来源**：本节的两条事实（T25 的五条写集 / 三处文本定位）出处 = `.superpowers/sdd/2026-09-13-frontend-redesign-batch8/controller-rulings.md` 的 §33.1 与 §33.4① —— 🔴 **该目录整体 gitignored、不入库**，故此处只给路径不给链接；**提交 sha 与写集已由 T21 用 `git show --name-only -1 a054ec73` 独立复核**。
...
> **🔻 批 6 收口就地加注 · 本表四行的落地（R6.7 · R6.9 · R14.7；2026-09-13，**上表原文与批 4 / 批 5 加注一字未改**）**：
> - 🔴 **`:836` 的「`docs/standards/` 新增动效规范章节（现对动效零命中）」= 批 6 已做**（批次归属的**就地消歧**）：落点 = [`docs/standards/motion.md`](../../standards/motion.md)（批 6 T1，提交 `bffed928`；**已进 `docs/standards/README.md` 索引**）。⚠️ 同一件事在本行与 [`v0.22.md:35`](../../versions/v0.22.md) 的「批 8 治理收口」行**两说**（`:35` 逐字写「新增动效规范章节」属批 8）⇒ 两处**按本加注读**：**规范章节本身已在批 6 落地**；**留批 8 的是** `docs/product/ui-ux-system.md` / `theme.md` 的**四层动效 / 三档强度回写**（**批 6 未写它们**，`progress.md` §一 的「🟡 待裁」仍在）。
> - **`docs/adr/ADR-032..035` 行**：**ADR-035 ✅ 已写**（批 6 T2，提交 `97295d64`；文件 `docs/adr/ADR-035-l4-motion-grammar-and-engine.md`，含**「后端契约例外」节**并在批 6 收口由 T35 **定稿回填**）⇒ 本行「ADR-034/035 **顺延至批 3 / 批 6**」**两件均已兑现**（ADR-034 批 4 补写 + ADR-035 批 6 落地）。**`docs/adr/README.md` 索引本批 +1 行**（T2）并在 T35 更正状态列措辞。
> - **`docs/adr/ADR-010-gap-filling-ai.md` 行**：批 1 已落（退役修订），批 6 **零动作**。
> - **`docs/versions/v0.21.md`（或 v0.20.14）行**：**批 6 ✅ 已落（2026-09-13）** = 「批 6 · 动效系统」节（**同七段结构**：交付 / 验收（八门禁终态 + 验收面逐条兑现度）/ 规格漂移纠正 / 过程中纠正的计划错误 / 诚实代价 / 未做登记（逐条带归属批次）/ 提交清单 `ac504b06^..975b3285` **含左端点 = 77 个提交**）。
> - **`docs/adr/ADR-019` 缺号**：🔴 **不补、不复用，只登记**（`docs/adr/README.md` 的「编号规则」逐字「编号一经分配不再复用」）；批 6 实盘最高 ADR = **ADR-035**。
> - ⚠️ **`docs/product/ui-ux-system.md` / `theme.md` 的四层动效 / 三档强度**：**本任务不写**（§14 上格逐字仍留批 8）⇒ 只在 `v0.22` 批 6 节的「未做登记」里点名。

> **🔻 批 8 终态就地加注 · 本表各行在批 8 的落地（2026-09-13 · T21；上表原文与批 4 / 批 5 / 批 6 加注一字未改）**：
> - 🔴 **`docs/versions/v0.21.md`（或 v0.20.14）行**：**批 8 ✅ 已落（2026-09-13）** = 「**批 8 · 治理收口与三条主线**」节（**同七段结构**：交付 / 验收（三条主线兑现度 + 八闸终态）/ 规格漂移纠正 / 过程中纠正的计划错误（E8-1 … E8-25）/ 诚实代价 / 未做登记（逐条带归属批次）/ 提交清单 `5852cdc4^..396b5e5c` **含左端点 = 63 个提交**）。🔴 **该节自带的三块**（**懒侧基线的两个时点与「重冻后总量 == 实测 ⇒ 零余量」的结构性发现** · **八闸第 ⑤ 闸的懒侧写 `⏭ 未判`** · **T15 / T15b 的两条实测边界**）是本批最需要下游读到的。
> - 🔴 **`docs/standards/` 行**（「新增动效规范章节（现对动效零命中）」）：**批 6 已落**（`docs/standards/motion.md`）⇒ 🔴 **批 8 未新增任何 `docs/standards/` 章节**；批 8 在该目录的动作只有**就地加注与数值同步**（`line-limit-exemptions.md` 的行数值与「312 条」散文 · `performance.md` 的「懒侧执行者缺口三段式」加注 · `motion.md` 的判据纪律小段 · `testing.md` 的门禁稳定性纪律）。
> - 🔴 **`docs/product/ui-ux-system.md` / `theme.md` 行**：**批 0-D 已落**原语层与阴影命名；🔴 **「四层动效 / 三档强度 / 断点与窗口 / §五§十 的旧参数」批 8 仍未写** ⇒ **登记为下一批输入**（批 8 只回写了「审校模式」那一节）。
> - **`docs/adr/ADR-032..035` 行**：**批 8 未新增任何 ADR**（实盘最高仍是 **ADR-035**）；批 8 的 ADR 动作 = **两处前向指针加注**（`ADR-034` / `ADR-035` **各纯插入 1 行、删除列 0**，指令落 `5018c7ab`）。
> - **`docs/adr/ADR-010-gap-filling-ai.md` 行**：批 1 已落（退役修订），**批 8 零动作**。
> - **`docs/standards/line-limit-exemptions.md` 行**：批 8 的终态 = `node scripts/line-limits.mjs --full` **exit 0**「`>600` 硬限 **0**（棘轮内）· 301–600 档 **121** · 登记条目 **121**」（T21 亲测，时点 **2026-09-13T17:32+08:00**）。
> - ⚠️ **`docs/product/requirements-pool.md` 行**（「REQ-201 修正（`update_fragment_group` 实际无调用方）· 新增 REQ 登记」）：🔴 **批 8 未动它**（`update_fragment_group` 的接线由**批 7** 的 T23 补齐）⇒ **登记**。
> - 🔴 **本表未列、批 8 新增的一行（归档面）**：`docs/tech-debt/` 按用户裁决 **U3 = a** 归档到 **`docs/archive/2026-09-13/`**（`335a6155` · `630d2bfa`）⇒ **`docs-check` 的扫描 / 检查数由 `282 / 182` 变为 `284 / 181`**（`282→284` / `182→181`，**已逐字登记在 `docs/archive/2026-09-13/README.md` 与 `docs/archive/README.md`**；T21 亲测现读数 = **`284 / 181`**）。
