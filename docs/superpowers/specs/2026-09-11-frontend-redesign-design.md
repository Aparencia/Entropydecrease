...
| 3 | 材质语言 | **D 显影**（四档墨度 = 确定度；未确认档 3.1:1 + 三条升档规则） |
...
| `--ink-4` 未确认 | `#909088` | `#6E6A62` | **3.1 / 3.2:1** —— 过渡态，见 4.3 |
| `--ink-3` 已重打分 | `#6E6E68` | `#9A958B` | 5.0 / 5.9:1 |
| `--ink-2` 已确认 | `#3A3A36` | `#D6D1C8` | 11.6 / 11.5:1（正文基准） |
| `--ink-1` 已改写 | `#1A1A1A` | `#F5F1E8` | ≈15:1；**唯一使用字重 +1 档的档位** |
| `--mark-clip` | `#F4F1E9` | `#221F1B` | 剪报底纹（默认开） |
| `--stamp` | `#B3271E` | `#E0604F` | **唯一非中性色**，只用于状态戳 |
| `--ok` 掌握 | `#2F7A4F` | `#4FAE74` | 掌握 / 已毕业 |
| `--due` 到期 | `#B26A12` | `#E0A44B` | 到期刻度 / 低置信点线 |
| `--link` | `#1F5FBF` | `#6E9BE8` | 链接 / 时间码 / 引用 |

> **对比度纪律**：亮档强调色**不可照搬暗档** —— 同一 `#17C3B2` 在暗底 8.6:1、在白底仅 2.2:1。故两档各给一值。
...
| 界面字体 | `"Inter", "Segoe UI Variable", "Microsoft YaHei UI", sans-serif`（标题/按钮/标签用黑体） |
| 等宽字体 | `"JetBrains Mono", Consolas, ui-monospace` + `tabular-nums`（时间码 / 元数据） |
| 字阶 | 25/600 · 17/600 · 15.5/1.9 · 13/20 · 12/18 · 11.5/16 mono（**下界 12px**） |
...
四档墨度在**浏览态**下：已确认 11.6:1 · 已重打分 5.0:1 · **未确认 3.1:1（低于正文 4.5:1 线）**。规则：
...
| 「某个具体的瞬间希望它是活的」 | 用户尚未回答；**留空不阻塞** —— 若给出则并入 8.6 |
...
| 未确认档 3.1:1 被误用 | 有人把关键信息放进未确认档 | 4.3 三条规则写进规范 + review 检查 |
