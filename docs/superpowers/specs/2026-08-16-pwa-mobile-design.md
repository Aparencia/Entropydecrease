...
3. **课堂助手**：两种形态——
   - 实时课堂：麦克风录音 → 转写 → AI 结构化笔记（参考 OPPO AI 笔记能力）
   - **视频转笔记**：导入视频文件（手机录屏/网课下载）→ 复用网关 `analyze-video` → 生成笔记
...
- 视频转笔记：`<input type="file" accept="video/*">` → `aiClient` multipart 上传 `/api/v1/multimodal/analyze-video` → 结构化笔记入库
...
| 工作项 | 工期 | 说明 |
|--------|------|------|
| 横切基建（PWA 构建/部署、移动端导航与布局、iOS Safari 细节、音频手势解锁） | 1–1.5 周 | VitePWA 已配置 |
| 番茄钟（UI 适配 + 通知流程） | 1–1.5 周 | 计时引擎已就绪（F3） |
| 空白笔记（编辑器移动适配、存储验证、剪藏/PDF 浏览器替代） | 2.5–3.5 周 | 最大单项是编辑器触控/键盘适配 |
| 课堂助手-录音转写（WebCaptureAdapter + 云端分段 ASR） | 3–4.5 周 | 60 处 IPC 中捕获链路需重写 |
| 课堂助手-视频转笔记（文件选择 + 上传 + 复用 analyze-video） | 0.5–1 周 | 后端现成（F6） |
| 桌面 ↔ 移动同步（账号绑定 + 浏览器端验证） | 1–2 周 | 底子已具备（F8） |
| 联调 + 移动端测试 + 打磨 | 1–1.5 周 | 双端回归 |
| **合计** | **约 10–15 周（≈2.5–3.5 人月）** | 2 人并行约 6–8 周 |
...
2. 番茄钟：切后台/锁屏 5 分钟再回来，计时不漂移（墙钟校准），到点通知可达
3. 空白笔记：新建/编辑/搜索/文件夹/链接在移动端可用；数据持久化于 IndexedDB
4. 课堂助手：麦克风录音 → 分段转写 → AI 生成结构化笔记全链路通；导入视频 → analyze-video → 笔记入库
5. 桌面 ↔ 移动：同一账号登录后数据双向增量同步，离线操作入队，恢复网络后补齐
6. 桌面端回归：Electron 现有功能不受影响（适配器模式，桌面路径不变）
7. 仓库门禁：`npm run check` 通过（lint/typecheck×2/test）
