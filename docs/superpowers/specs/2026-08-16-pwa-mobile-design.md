...
> 日期：2026-08-16 ｜ 状态：待评审（v4：音频源决策——录屏导入含系统音频为主力，麦克风为应急；真·系统音频监听列 Phase 2 原生壳） ｜ 分支：dev
...
3. **课堂助手**：三种形态——
   - 实时课堂：麦克风录音 → 转写 → AI 结构化笔记（**应急通道**；近似桌面端"实时录制识别"机制）
   - 视频转笔记（**主力通道**）：导入视频文件（**手机系统录屏——含系统音频**/网课下载/抖音保存的视频）→ 复用网关 `analyze-video` → 生成笔记。系统录屏是移动端"监听系统扬声器"的离线等价物（PWA 无法实时监听系统输出音频，见 §3.5）
...
└── WebCaptureAdapter        ← 新增：麦克风实时转写（应急通道）+ 系统录屏导入转写（主力通道）
```

- **音频源决策（v4）**：PWA 无法实时监听系统扬声器（iOS 无 API；Android 网页无法捕获原生 App 音频）。主力通道 = 手机系统录屏（系统音频随录屏文件落盘）→ 导入 → analyze-video 提取转写；应急通道 = 麦克风实时转写（外放收音，音质受限）。真·系统音频实时监听（Android AudioPlaybackCapture / iOS ReplayKit）列为 Phase 2 原生壳。
...
| 系统音频环回 | PWA 无 API 无法实时监听；替代：**系统录屏导入（含系统音频）为主力** + 麦克风应急；Phase 2 原生壳（Android AudioPlaybackCapture / iOS ReplayKit） |
...
| 课堂助手-录音转写（WebCaptureAdapter 麦克风应急通道 + 转写链路） | 2.5–3.5 周 | 主力为系统录屏导入（走视频导入链路），麦克风为应急 |
...
| **合计** | **约 9–12 周（≈2–2.5 人月）** | 2 人并行约 5–7 周 |
...
5. 课堂助手：系统录屏导入（含系统音频）→ analyze-video → 结构化笔记全链路通（主力通道）；麦克风实时转写可用（应急通道）；粘贴抖音链接出现引导流程并可完成导入
