...
**（c）追加记录 · 非行号类的两处错误（控制方 2026-09-13 追加，体例同 §C12「保留错误记录、勿抹掉」）**
...
| **E-25** | `nativeButton.ratchet.test.ts:268-273`（出现于 `## Global Constraints` 的 §C5.1 行 / **T10 的 V1** / **`## 陷阱` B4** 三处） | 🔴 **越界**：该文件**只有 265 行**（`countLines()`）。真锚 = **`:199-200`**（`expect(SCAN.buttonTotal, …).toBeLessThanOrEqual(FROZEN_NATIVE_BUTTON_TOTAL)` = 「常量 == Σ entries」的牙）；同文件的另两条牙 = 逐文件表 **`:203-214`**（✓ 本就正确）、`const *Btn*` 族 **`:255-263`**（✓ 本就正确） | `nativeButton.ratchet.test.ts`（**265**）· `probe6.mjs`（逐行打印 `:200-265`；并全文件扫 `FROZEN_NATIVE_BUTTON_TOTAL` ⇒ 仅 `:35` / `:199` / `:200`）· 出处 = **控制方 §C9.7 与 §C5.1 的引述**（控制方侧同一错锚） | `## Global Constraints` 的 C5.1 行 · **T9 的 Step 3 表** · **T10 的 V1（M1）** · `## 陷阱` B4 |
...
- [ ] **Step 3: 🔴 九个棘轮的「普查 vs 棘轮」逐处声明（C9.7）**
  ...
  | `nativeButton` 的总量 | 🔴 **刻意设计的棘轮**（`:9-11` 逐字解释：拆件会让逐行 key 假红，故用计数 + `SPLIT_MOVES` 守恒） | 保留 `<=` | 🔴 **不得改成等式**；它的**等式牙齿已经在**：逐文件表（`:203-214`）+ 常量 == Σ entries（**真值 `:199-200`** —— 🔴 §C9.7 原引的 `:268-273` **越界**（该文件 **265** 行），见 `### 表 6b` 的 **E-25**） |
  | 新增的懒侧三判据 | **棘轮**（族上限 + 总上限） | `<=` | — |
...
- [ ] **Step 2: 自证「只改散文」**
  ...
  1. 🔴 **该提交只改字符串** —— **任何断言、计数、常数一律不动**（`nativeButton.ratchet.test.ts` 的 `FROZEN_*` / `*_BY_FILE` / `SPLIT_MOVES` / 「常量 == Σ entries」全在冻结面内）；
...
| # | 判据 | 变异体（专属） | 期望 |
|---|---|---|---|
| **V1** | **三支棘轮判据全绿** · `--full` **exit 0** · `docs-check` **exit 0** | **M1**：把 `FROZEN_NATIVE_BUTTON_TOTAL` 从 393 改成 394（**动常数**） | **M1 后期望**：`nativeButton.ratchet.test.ts` 的「常量 == Σ entries」红 —— 🔴 **真值 = `:199-200`**（`expect(SCAN.buttonTotal, …).toBeLessThanOrEqual(FROZEN_NATIVE_BUTTON_TOTAL)`）；原写 `:268-273` **越界**（该文件 **265** 行）⇒ 见 `### 表 6b` 的 **E-25**。**证明"只改散文"这条边界真的被判** |
...
- **③ 禁止只改常量不动逐文件表**（`nativeButton.ratchet.test.ts:203-214` 的逐文件表 + **`:199-200`** 的「常量 == Σ entries」是守卫已有的牙；🔴 §C5.1 原引的 `:268-273` **越界**，见 `### 表 6b` 的 **E-25**）。
