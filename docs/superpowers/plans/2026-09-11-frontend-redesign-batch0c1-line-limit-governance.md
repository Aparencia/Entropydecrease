...
> **两点行为说明（写下来避免将来困惑）**：
> 1. **lint-staged 会附带暂存文件名**，脚本予以忽略；且 lint-staged 在**部分暂存**的情况下会把未暂存的改动 stash 起来再跑命令 ⇒ 校验看到的是**"即将提交的状态"**（暂存文件 = 暂存内容，其余 = `HEAD`），**不一定等于你编辑器里的当前工作树**。这是正确行为，但若你看到行数与眼前不符，先想这一点。
...
> **执行结果（2026-09-11 收尾时补记）**：Task 0–3 已全部交付。工具现为**绿**（默认与 `--full` 都 `exit=0`，stderr 0 字节），登记表 **138 条 / 179 行**、历史节 16 条、占位符 0、脚本 250 行。Task 4（接线 lint-staged + CI）与 Task 5（回写门槛与验收）待做。
