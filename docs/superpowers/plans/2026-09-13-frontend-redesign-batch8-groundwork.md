...
- [ ] **Step 1: 串行跑八闸**（**逐条、不并发**；命令与期望见 `### 表 1`）：
  ...
  🔴 **Rust 不跑**（**段 8a 零 Rust 改动** ⇒ 报告须给 `git log --oneline <8a 起点>..HEAD -- app/src-tauri` 为空的证据；⚠️ **全批有 Rust 改动（T25/T27/T28）⇒ 那是 8e 的事**）。
...
| **V2** | `line-limits --full` 读数与 8a 开工基线**逐字相同**（`0 / 121 / 121`） | **M2**：在导出副本里给任一文件加 1 行 | **M2 后期望**：`301–600 档` **121 → 122** 且 `(e)` 判据报「行数不一致」（**具名**）⇒ `exit 1` |
...
**诚实边界**：① 本任务**不跑真构建** ⇒ **懒侧预算未判**（`⏭`），**不得**写成「懒侧达标」；② `cargo` **不跑**（**段 8a 零 Rust 改动**）⇒ Rust 侧读数仍是**批 7 的引用值**（⚠️ 全批的 Rust 面由 T22 覆盖）；③ V4 无机器牙（段边界是**文本主张**）。
...
| **V5** | **新文件的冻结键**：`noteEvidenceModel.ts` / `noteEvidence.fixtures.ts` 在两个域内的**命中数全为 0**（`frozen.mjs` 式自测：mutedGray / fontOob / border / radius / shadow / nativeBtn **各 0**） | **M5**：往 `fixtures.ts` 里塞一行 `fontSize: 11`（越界档） | **M5 后期望**：`textRatchet.test.ts` 的 `FROZEN_FONT_OOB_TOTAL` 一致性判据红（**它报的具名断言 = `冻结表的逐文件之和 ≠ 冻结总数`**）⇒ 证明「新文件出现字面量就会红」 |
...
| **V5** | **冻结键全 0**（两个新件 + `App.tsx` 无新增） | **M5**：在 `proofreadMode.ts` 里加一行 `fontSize: 11` | **M5 后期望**：`textRatchet.test.ts` 的 Σ 一致性判据红（**具名**） |
...
| # | 判据 | 变异体（专属） | 期望 |
|---|---|---|---|
| **V1** | **行数三件**：`NotesPage.tsx` **≤300** · `noteViews.test.tsx` **≤300** · `NotesReadingColumn(±).tsx` **≤300**；且 `NotesPage.tsx` 的净增 **≤ +5** | **M1**：在 `NotesPage.tsx` 里内联写取数逻辑（把 hook 摊平） | **M1 后期望**：行数 >300 ⇒ `line-limits --full` 的 `(d)` 判据红（`>300 未登记`，**具名输出**） |
| **V2** | **IPC 面零变化**：`check-command-registry` **311/311/0**；`git log --oneline <8b 起点>..HEAD -- app/src-tauri` **为空** | **M2**：在 `useNoteEvidence` 里改调一个不存在的命令名 | **M2 后期望**：行为判据的 `invoke` 断言红（**具名**：命令名逐字不符）；`check-command-registry` **仍绿** ⇒ **证明这道判据不是 registry 的重复** |
| **V3** | **新 hook 的冻结键全 0** | **M3**：在 hook 里加 `fontSize: 11` | **M3 后期望**：`textRatchet.test.ts` 的 Σ 一致性红（**具名**） |
...
| # | 判据 | 变异体（专属） | 期望 |
|---|---|---|---|
| **V1** | **D3 规范形（§C55.2）**：每个文件的「**剥注释后去纯空白行**」规范形比较 ⇒ **改动行 = 0 / 0**；🔴 **必须同时给「纯空白行增删数」**（承 §C61.2：**删除 = 0**） | **M1**：把 `surfaceResidual.ts:101` 的 `count: 1` 改成 `count: 0` | **M1 后期望**：D3 规范形报「非注释改动 = 1」⇒ 红；**且** `surfaceRatchet.test.ts` / `surfaceTagRegistry.test.ts` 的登记一致性判据红（**两条独立证据**） |
...
| **V3** | **IPC 面零变化**：`check-command-registry` **311/311/0**；`git diff` 对 `app_commands.rs` / `db_migrations.rs` **零 hunk** | **M3**：把 `provider_scope` 的默认值改成新命令 | **M3 后期望**：registry 三向一致性红（**具名**） |
...
### C · 本批新增陷阱（P-35 – P-40，**本计划者实测 / 控制方 §2 G5 登记**）
