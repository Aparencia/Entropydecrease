/**
 * TaskPresetCard — 潜航任务（预设管理卡）
 *
 * 覆盖原 PresetManager 全部能力：预设列表/新建/编辑/删除/排序。
 * 内置预设标注「内置」，不可删除。
 *
 * @ai-context: 深潜设置页改造——潜航任务卡。
 */
...
export function TaskPresetCard({ presets, canCreate, onDelete, onMove, onSavePreset }: TaskPresetCardProps) {
    ...
    <div className="rounded-kb-lg border border-border/40 bg-bg-secondary/60 p-kb-md">
      <div className="flex items-center justify-between mb-kb-sm">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-h3 font-medium text-text-primary">潜航任务</h2>
        </div>
        {canCreate && (
          <Button
            variant="secondary"
      ...
      <p className="text-c1 text-text-tertiary mb-kb-sm">
        每个任务拥有独立的节律参数（最多 {presets.length}/6 个）
      </p>
        ...
        {presets.map((preset, index) => (
          ...
          >
            ...
            <div className="flex items-center gap-1">
              ...
              <Tip text={preset.builtin ? '内置任务不可删除' : '删除任务'}>
                <button
