 ...
 *   已有 **5 处属性落点**（批 7 T10 实测；口径 = 剥注释后计 `data-tone=` 属性形态、域 = `app/src` 减 `*.test.*`；
 *   T35b 当时的 4 处系同一口径、第 5 处由 T20 新增）：`components/review/DueScale.tsx:134`（T30，条件式 `instrument`）·
 *   `components/review/ReviewSessionPanel.tsx:252`（T30，`instrument`）· `views/session/SessionTriTrackView.tsx:282`（T27，`instrument`）·
 *   `components/session-detail/SessionRawView.tsx:108`（T28，`paper`）· `components/TranscriptRow.tsx:104`（T20，`instrument`）—— 旧措辞
 *   「生产 0 命中」描述的是 T8 当时的真相，**已过期**；本文件仍只造**游离元素**验基调契约；
