 ...
 * 边界：**不许从「首屏静态可达」的模块静态引入本模块**（判据 = 上面那条**闭包交集**，不是白名单）；
 *   **惰性链上的模块静态引入是安全的** —— 旧措辞「本模块**只能经 `await import()` 到达**」是**过宽**的，
 *   已按 R60.1 更正 R41.3 的适用范围（先例 ✅：`motion/controls.ts:49` 今天就静态引入本模块，而该件
 *   ∉ 首屏静态闭包（89 文件）⇒ 守卫绿 —— T35d 用 `engine.guard.test.ts` 自己的仪器实测；这与
 *   `motion/controls.ts` 头注「使用纪律」是同一条纪律，两处口径一致）。
 *   凡需要 `useGSAP` 的组件，其自身必须在**惰性链**上：经 `await import()` 到达，或经惰性视图 /
 *   注册表驱动的静态引入到达 —— 两种形态都不产生首屏静态边。
