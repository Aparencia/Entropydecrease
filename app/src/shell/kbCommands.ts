 ...
 *   （本仓 vitest 全局 `environment: "node"`，`invoke` 不可用）。组件只负责调用与渲染，不负责数据形状
 *   —— 这样 T12 的测试面是 100%，而不是 0%。
 *
 ...
 *     · `limit` 缺省 `KB_SEARCH_DEFAULT_LIMIT = 10` 且 `.min(KB_SEARCH_MAX_LIMIT = 50)`。
 ...
 *
 * 边界：本模块**不 import 任何 React**（纯逻辑，V3 判据；也因此可脱离 Tauri 运行时单测）；
 *   不做排序（顺序由后端给）· 不做中文分词（Rust 侧 `kb_fts.rs` 已把中文规划成 trigram）·
 *   不碰任何 `focus*` 状态（只产出「跳转意图」，落到状态机是 `App.tsx` 的事）。
 */
...
/** Rust `KB_SEARCH_MAX_LIMIT` 的前端镜像（命令层已 clamp，这里再夹一次防越界入参） */
...
/**
 * 检索 → 结果命令（带降级标记；面板用的是这个版本 —— 「空结果」与「检索失败」必须可区分）。
...
export async function kbSearchCommands(
  ...
  if (!query) return { commands: [], degraded: false };
  ...
  try {
    ...
    return { commands: commandsFromHits(Array.isArray(hits) ? hits : [], onJump), degraded: false };
  } catch (e) {
    console.warn("[kbCommands] kb_search 失败——⌘K 降级为仅页面命令:", describeError(e));
    return { commands: [], degraded: true };
  }
}
