 ...
 * 边界：**只管宽度与折叠**；不描述列的内容、组件的挂载、也不描述列的可见性 ——「采集态整列隐藏」（规格
 *   §6.2 课堂源列行）仍挂在**批 6 的相变两态**名下且**尚未落地**：`app/src` 内 `data-shell-phase` 零命中、
 *   `shell/shellPhase.ts` 尚不存在，批 6 计划里承接相变的是相位通道 + 顶栏相位 CSS（「列隐藏」不在其文本内）；
 *   ⚠️ 规格两处口径不一（§6.2 该行写「整列隐藏」，§6.3 相变态表写「源列保留（采集配置面）」）—— 本模块
 *   不裁决可见性，两个口径都不由这里兑现 ⇒ 别把这条登记读成「已交付」。
 */
