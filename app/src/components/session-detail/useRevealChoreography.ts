 ...
 *   **一个 `opacity` 都不写**（两个动效谁都不许覆盖谁）。⚠️ **两条落点是「同段不同元素」而不是同一个
 *   元素**（与计划 `:1995` 的措辞不同，理由见 `task-28-report.md`）：本件动**段落行**（`[data-seg-id]`
 *   的 `<div>`），R11.3 的类名在行内的正文 `<Text>`（`<span>`，`display: inline`）—— 而 **`transform`
 *   对行内元素无效**（CSS Transforms L1 的 transformable element 不含 `display:inline`）⇒ 把显影挂在
 *   正文上会是一条**看不见的动画**。改挂段落行后：不同元素 **且** 不同属性 ⇒「互不覆盖」比计划更强。
 *   「显影」的可见通道因此是**位移**（`y: +8px → 0` 的逐段升起），墨度通道留给环境层。
 *
