  ...
  return (
    <div>
      ...
      {tasks.length === 0 ? (
        ...
        <Surface level="none" radius="panel" style={{ maxHeight: 200, overflowY: "auto" }}>
          {tasks.map((t) => {
            ...
            return (
              <div key={t.taskId} style={{ display: "flex", gap: 8, alignItems: "center", padding: "4px 8px", borderBottom: "1px solid #f3f4f6", fontSize: 11 }}>
                ...
                {t.state === "succeeded" && !t.adopted && (
                  <Button variant="secondary" size="sm" onClick={() => void viewResult(t)}>查看</Button>
                )}
                {t.state === "succeeded" && t.adopted && (
      ...
      {selected && summary && (
        <div style={{ border: "1px solid #a7f3d0", background: "#ecfdf5", borderRadius: 6, padding: 8, marginTop: 6, fontSize: 11 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            ...
            <Button variant="secondary" size="sm" onClick={() => { setSelected(null); setSummary(null); }}>关闭</Button>
          </div>
        </div>
      )}
