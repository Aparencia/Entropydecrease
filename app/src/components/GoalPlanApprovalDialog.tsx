...
export default function GoalPlanApprovalDialog({ view, onConfirm, onClose, onUseRules }: Props) {
  ...
  return (
    /* 批 4 T5：自建遮罩 + 面板几何 + 手写标题/关闭（原 `:79-84`）交给 `Modal`。
       档位 `l`(720) 对齐规格 §5.2 的"向导/工作台"，原面板 560 ⇒ **+160 px**（已登记为可见变化）。
       原实现没有 ESC 路径，迁移后新增（同遮罩点击）；本组件在 `InterviewDialog` 内嵌套渲染，
       `Modal` 的 React 树深度机制会把它认成内层（ESC 只关这一层）—— 见报告。 */
    <Modal
      open
      onClose={onClose}
