//! AI mock 适配器（REQ-085 文本复核 / REQ-141 精修 / REQ-142 知识补充）。
//!
//! @ai-context: 本地"假 AI"——离线/测试环境按规则产出**合法**响应，持续验证
//!              各协议 schema 校验链（TextFilterResponse::validate /
//!              AiRefineResponse::validate / AiEnrichResponse::validate）；
//!              云端实装（V1.0）只需替换本适配器实现，协议/护栏零改动。
//! @ai-context: review_text 为 REQ-085 文本复核的 mock 三态判定（规则化）。

use crate::ai_protocol::{
    TextFilterAction, TextFilterDecision, TextFilterRequest, TextFilterResponse,
};
