//! 帧处理工具与调度单测（AAA 模式；纯函数，无 IO 依赖）。
//!
//! @ai-context: 由 frame_diff.rs 以 #[cfg(test)] #[path] 引入，保持实现文件 ≤300 行。
//! @ai-context: 变化检测（FrameDiffDetector）已随 ADR-011 移除，测试迁至 grid_diff_tests.rs。

use crate::capture::frame_diff::{
    bottom_quarter_rect, downscale_bgra, DualRateScheduler, Rect, SampleRegion,
};
