//! 笔记文本 AI 复核 Tauri commands（REQ-085 / v0.6.0 M1）。
//!
//! @ai-context: 本层只做参数校验、调用适配器（ai_text_filter 云端 / ai_mock 离线
//!              mock）、错误映射（AGENTS.md §6）；未授权/云端不可用/超配额/
//!              非法响应一律降级为本地纯规则结果（不上传）。
...
use crate::ai_protocol::{
    TextFilterDecision, TextFilterRequest, TextFilterResponse, TextFilterSegment,
};
