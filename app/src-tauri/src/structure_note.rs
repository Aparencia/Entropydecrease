...
//!
//! @ai-context: 纯本地"结构组织"——章节边界入笔记（## 标题层级）。v0.11.5
//!              （spec 8️⃣）：词汇表完全移出笔记（术语表改在会话详情展示，
//!              新命令 session_glossary）；与 v0.8.0 AI 精修（REQ-141）并存：
//!              本层是规则版结构生成，AI 版将来以结构块数组重写本层输出
//!              （精修=整理不创作，语义不变）。
//! @ai-context: 叠加在 note_filter **之后**（方案 B：独立渲染层）——filter_note
//!              过滤链零改动零回归；预览/落库双出口同口径由命令层调用顺序保证。
//! @ai-context: 诚实降级：无章节（口播档案）不插标题；配置全关原样返回。
//!              边界落在段内不切段（粗粒度边界，不伪造粒度）。
...
/// v0.11.5：glossary_block/glossary_max_terms 移除（词汇表完全移出笔记）——
/// 旧配置文件多余字段被 serde 默认忽略，无需迁移。
...
pub struct StructureStats {
    ...
    /// 词汇表条目数（v0.11.5 词汇表移出笔记后恒 0，保留字段兼容旧 JSON）
...
/// @ai-context: 全关（chapter_headings=false）原样返回零统计——v0.7.5 输出
///              逐字节一致（零回归护栏）。
