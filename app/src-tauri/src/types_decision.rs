//! @ai-context: 知识决策/应用记录（v0.13.3 REQ-208，一表两面）与其 used_refs 解析辅助（M1 预埋）。
//! @ai-context: 同文件另含 v0.13.8/v0.14.1 画布契约（节点位置/视口/偏好）——同属知识子系统的视图层契约，
//!              合并于此以满足单文件 ≤300 行（AGENTS.md §3）：知识体系本体已占 types_knowledge.rs 的地板。
