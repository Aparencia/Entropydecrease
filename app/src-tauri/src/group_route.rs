...
///              2. 高密度 ≥2 票共振 → 自成一组；
///              3. 高低票并存（冲突）→ 待确认；
///              4. 低密度 ≥2 票共振 → 有领域归主题组，无领域待确认；
///              5. 弱/零信号 → 自成一组兜底（独立组可后续移动，不埋没）。
pub fn route_group(signals: &GroupRouteSignals) -> GroupRouteDecision {
    ...
    if low >= MIN_RESONANT_VOTES {
        return match signals.domain_kind {
            Some(kind) => GroupRouteDecision {
                action: RouteAction::TopicGroup,
                reasons: {
                    reasons_low.push(format!("领域命中（{}）", kind.label()));
                    reasons_low
                },
            },
            // 低结构但无领域——主题组无处可归，诚实待确认（不硬塞大类抽屉）
            None => GroupRouteDecision { action: RouteAction::NeedConfirm, reasons: reasons_low },
        };
    }
