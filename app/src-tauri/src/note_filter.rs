...
use crate::types::{SessionOcrBlock, SessionSegment, TranscriptSegment};
...
pub fn filter_note(
    ...
    // ② 画面要点（水印 + UI 垃圾 + 低分 + 去重）
    let ocr_points = build_ocr_points(ocr_blocks, ui_junk);
    let markdown = rebuild_markdown(title, &kept, &ocr_points);
    NoteFilterResult { title: title.to_string(), markdown, kept, ocr_points, stats, filtered, merged }
}
...
/// 画面要点构建（纯函数）：水印排除 + UI 垃圾 + 低分过滤 + 精确去重。
fn build_ocr_points(ocr_blocks: &[SessionOcrBlock], ui_junk: &UiJunkList) -> Vec<String> {
    // 水印检测（REQ-059）：全帧块（字幕区文本已在讲述内容中，不重复进要点）
    let inputs: Vec<WatermarkInput> = ocr_blocks
        .iter()
    ...
    let mut sorted: Vec<&SessionOcrBlock> = ocr_blocks
        .iter()
        .filter(|b| {
            b.region == "full"
                && b.score >= OCR_MIN_SCORE
    ...
    for b in sorted {
        ...
        if seen.insert(text.clone()) {
            points.push(format!("[{}] {}", crate::concat::format_timestamp(b.timestamp_ms), text));
        }
    }
    points
}
