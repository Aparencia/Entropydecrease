#!/usr/bin/env node
/**
 * @ai-context 行数红线的**门禁入口**（AGENTS.md §3.1）；口径与扫描实现已拆至 `./lib/lineScan.mjs`（批 8 T1），原 7 个导出名**全部在原路径 re-export** ⇒ 模块导出面逐字不变、既有引用零改动。
 * Why 拆件：本件原为单件 300 行（贴 ≤300 硬线、余量 0），而「末尾换行盲区」判据（批 8 T2）必须落在本件内
 * ⇒ 纯函数与表解析搬进 `lib/`，本件只留进程级副作用（argv / `console` / `process.exit` / 主入口），承 AGENTS.md §3.1「纯逻辑与副作用物理分离」。
...
export { countLines, scanTree, parseTable, FROZEN_OVER_LIMIT, HARD_LIMIT, SOFT_LIMIT, TABLE_PATH } from './lib/lineScan.mjs';
